...
typedef enum
{
  TI_JSON_TOKEN_NONE, ///< There is no value (as distinct from #TI_JSON_TOKEN_NULL).
  TI_JSON_TOKEN_BEGIN_OBJECT, ///< The token kind is the start of a JSON object.
  TI_JSON_TOKEN_END_OBJECT, ///< The token kind is the end of a JSON object.
  TI_JSON_TOKEN_BEGIN_ARRAY, ///< The token kind is the start of a JSON array.
  TI_JSON_TOKEN_END_ARRAY, ///< The token kind is the end of a JSON array.
  TI_JSON_TOKEN_PROPERTY_NAME, ///< The token kind is a JSON property name.
  TI_JSON_TOKEN_STRING, ///< The token kind is a JSON string.
  TI_JSON_TOKEN_NUMBER, ///< The token kind is a JSON number.
  TI_JSON_TOKEN_TRUE, ///< The token kind is the JSON literal `true`.
  TI_JSON_TOKEN_FALSE, ///< The token kind is the JSON literal `false`.
  TI_JSON_TOKEN_NULL, ///< The token kind is the JSON literal `null`.
} ti_json_token_kind;

/**
 * @brief A limited stack used by the #ti_json_writer and #ti_json_reader to track state information
 ...
 *
 * @remarks An instance of #ti_json_token must not outlive the lifetime of the #ti_json_reader it
 * came from.
 */
typedef struct
{
  ...
  /// The user can call #ti_json_token_copy_into_span() to get the token value into a contiguous
  /// buffer.
  ...
  /// This read-only field gives access to the type of the token returned by the #ti_json_reader,
  ...
  struct
  {
    ...
    /// its set to #TI_SPAN_EMPTY.
    ti_span* pointer_to_first_buffer;
...
/**
 * @brief Copies the content of the \p token #ti_json_token to the \p destination #ti_span.
 *
 * @param[in] json_token A pointer to an #ti_json_token instance containing the JSON text to copy to
 * the \p destination.
 * @param destination The #ti_span whose bytes will be replaced by the JSON text from the \p
 * json_token.
 *
 ...
 *
 * @param[in] json_token A pointer to an #ti_json_token instance.
 ...
 * @retval #TI_OK The boolean value is returned.
 * @retval #TI_ERROR_JSON_INVALID_STATE The kind is not #TI_JSON_TOKEN_TRUE or #TI_JSON_TOKEN_FALSE.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The number is returned.
 * @retval #TI_ERROR_JSON_INVALID_STATE The kind is not #TI_JSON_TOKEN_NUMBER.
 * @retval #TI_ERROR_UNEXPECTED_CHAR A non-ASCII digit is found within the \p json_token or \p
 ...
 * @retval #TI_OK The number is returned.
 * @retval #TI_ERROR_JSON_INVALID_STATE The kind is not #TI_JSON_TOKEN_NUMBER.
 * @retval #TI_ERROR_UNEXPECTED_CHAR A non-ASCII digit is found within the token or if it contains a
 ...
 * @retval #TI_OK The number is returned.
 * @retval #TI_ERROR_JSON_INVALID_STATE The kind is not #TI_JSON_TOKEN_NUMBER.
 * @retval #TI_ERROR_UNEXPECTED_CHAR A non-ASCII digit is found within the token or if it contains
 ...
 * @retval #TI_OK The number is returned.
 * @retval #TI_ERROR_JSON_INVALID_STATE The kind is not #TI_JSON_TOKEN_NUMBER.
 * @retval #TI_ERROR_UNEXPECTED_CHAR The resulting \p out_value wouldn't be a finite double number.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The string is returned.
 * @retval #TI_ERROR_JSON_INVALID_STATE The kind is not #TI_JSON_TOKEN_STRING.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE \p destination does not have enough size.
 */
...
/**
 * @brief Determines whether the unescaped JSON token value that the #ti_json_token points to is
 * equal to the expected text within the provided byte span by doing a case-sensitive comparison.
 *
 * @param[in] json_token A pointer to an #ti_json_token instance containing the JSON string token.
...
/**
 * @brief Allows the user to define custom behavior when writing JSON using the #ti_json_writer.
 */
typedef struct
{
  struct
 ...
 *
 * @details Call this to obtain an initialized #ti_json_writer_options structure that can be
 * modified and passed to #ti_json_writer_init().
 *
 * @return The default #ti_json_writer_options.
 */
TI_NODISCARD TI_INLINE ti_json_writer_options ti_json_writer_options_default()
{
  ti_json_writer_options options = {
    ._internal = {
      .unused = false,
    },
  };
 ...
 * buffer.
 *
 * @remarks #ti_json_writer builds the text sequentially with no caching and by default adheres to
 * the JSON RFC: https://tools.ietf.org/html/rfc8259.
 */
typedef struct
{
  /// The total number of bytes written by the #ti_json_writer to the output destination buffer(s).
  ...
  struct
  {
    ...
    ti_json_token_kind token_kind; // needed for validation, potentially #if/def with preconditions.
    ...
    _ti_json_bit_stack bit_stack; // needed for validation, potentially #if/def with preconditions.
...
/**
 * @brief Initializes an #ti_json_writer which writes JSON text into a buffer.
 *
 * @param[out] out_json_writer A pointer to an #ti_json_writer instance to initialize.
 * @param destination_buffer An #ti_span over the byte buffer where the JSON text is to be written.
 * @param[in] options __[nullable]__ A reference to an #ti_json_writer_options
 * structure which defines custom behavior of the #ti_json_writer. If `NULL` is passed, the writer
 * will use the default options (i.e. #ti_json_writer_options_default()).
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK #ti_json_writer is initialized successfully.
...
/**
 * @brief Initializes an #ti_json_writer which writes JSON text into a destination that can contain
 * non-contiguous buffers.
 *
 * @param[out] out_json_writer A pointer to an #ti_json_writer the instance to initialize.
 * @param[in] first_destination_buffer An #ti_span over the byte buffer where the JSON text is to be
 * written at the start.
 * @param[in] allocator_callback An #ti_span_allocator_fn callback function that provides the
 ...
 * through to calls to the #ti_span_allocator_fn.
 * @param[in] options __[nullable]__ A reference to an #ti_json_writer_options
 * structure which defines custom behavior of the #ti_json_writer. If `NULL` is passed, the writer
 * will use the default options (i.e. #ti_json_writer_options_default()).
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The #ti_json_writer is initialized successfully.
...
/**
 * @brief Returns the #ti_span containing the JSON text written to the underlying buffer so far, in
 * the last provided destination buffer.
 *
 * @param[in] json_writer A pointer to an #ti_json_writer instance wrapping the destination buffer.
 *
 * @note Do NOT modify or override the contents of the returned #ti_span unless you are no longer
 * writing JSON text into it.
 *
 * @return An #ti_span containing the JSON text built so far.
 *
 ...
 * non-contiguous buffers (using #ti_json_writer_chunked_init()), and the JSON is larger than the
 ...
 */
TI_NODISCARD TI_INLINE ti_span
ti_json_writer_get_bytes_used_in_destination(ti_json_writer const* json_writer)
{
  return ti_span_slice(
 ...
 *
 * @param[in,out] ref_json_writer A pointer to an #ti_json_writer instance containing the buffer to
 * append the string value to.
 * @param[in] value The UTF-8 encoded value to be written as a JSON string. The value is escaped
 * before writing.
 *
 * @note If you receive an #TI_ERROR_NOT_ENOUGH_SPACE result while appending data for which there is
 ...
 *
 * @remarks If \p value is #TI_SPAN_EMPTY, the empty JSON string value is written (i.e. "").
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The string value was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The provided \p json_text was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The destination is too small for the provided \p json_text.
 * @retval #TI_ERROR_JSON_INVALID_STATE The \p ref_json_writer is in a state where the \p json_text
 * cannot be appended because it would result in invalid JSON.
 * @retval #TI_ERROR_UNEXPECTED_END The provided \p json_text is invalid because it is incomplete
 * and ends too early.
 * @retval #TI_ERROR_UNEXPECTED_CHAR The provided \p json_text is invalid because of an unexpected
 * character.
 */
TI_NODISCARD ti_result
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The property name was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
TI_NODISCARD ti_result
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The boolean was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The number was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
 ...
 * @retval #TI_OK The number was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 * @retval #TI_ERROR_NOT_SUPPORTED The \p value contains an integer component that is too large and
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK `null` was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK Object start was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 * @retval #TI_ERROR_JSON_NESTING_OVERFLOW The depth of the JSON exceeds the maximum allowed
 * depth of 64.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK Array start was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 * @retval #TI_ERROR_JSON_NESTING_OVERFLOW The depth of the JSON exceeds the maximum allowed depth
 * of 64.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK Object end was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK Array end was appended successfully.
 * @retval #TI_ERROR_NOT_ENOUGH_SPACE The buffer is too small.
 */
...
/**
 * @brief Allows the user to define custom behavior when reading JSON using the #ti_json_reader.
 */
typedef struct
{
  struct
...
} ti_json_reader_options;

/**
 * @brief Gets the default json reader options which reads the JSON strictly according to the JSON
 * RFC.
 *
 * @details Call this to obtain an initialized #ti_json_reader_options structure that can be
 * modified and passed to #ti_json_reader_init().
 *
 * @return The default #ti_json_reader_options.
 */
TI_NODISCARD TI_INLINE ti_json_reader_options ti_json_reader_options_default()
{
  ti_json_reader_options options = {
    ._internal = {
      .unused = false,
    },
  };
 ...
 *
 * @remarks The token field is meant to be used as read-only to return the #ti_json_token while
 * reading the JSON. Do NOT modify it.
 */
typedef struct
{
  /// This read-only field gives access to the current token that the #ti_json_reader has processed,
  /// and it shouldn't be modified by the caller.
  ti_json_token token;
...
/**
 * @brief Initializes an #ti_json_reader to read the JSON payload contained within the provided
 * buffer.
 *
 * @param[out] out_json_reader A pointer to an #ti_json_reader instance to initialize.
 * @param[in] json_buffer An #ti_span over the byte buffer containing the JSON text to read.
 * @param[in] options __[nullable]__ A reference to an #ti_json_reader_options structure which
 * defines custom behavior of the #ti_json_reader. If `NULL` is passed, the reader will use the
 * default options (i.e. #ti_json_reader_options_default()).
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The #ti_json_reader is initialized successfully.
 * @retval other Initialization failed.
 *
 * @remarks The provided json buffer must not be empty, as that is invalid JSON.
 *
 * @remarks An instance of #ti_json_reader must not outlive the lifetime of the JSON payload within
 * the \p json_buffer.
 */
 ...
 * json_buffers array.
 * @param[in] options __[nullable]__ A reference to an #ti_json_reader_options
 * structure which defines custom behavior of the #ti_json_reader. If `NULL` is passed, the reader
 * will use the default options (i.e. #ti_json_reader_options_default()).
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The #ti_json_reader is initialized successfully.
 * @retval other Initialization failed.
 ...
 *
 * @param[in,out] ref_json_reader A pointer to an #ti_json_reader instance containing the JSON to
 * read.
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The token was read successfully.
 * @retval #TI_ERROR_UNEXPECTED_END The end of the JSON document is reached.
 * @retval #TI_ERROR_UNEXPECTED_CHAR An invalid character is detected.
 * @retval #TI_ERROR_JSON_READER_DONE No more JSON text left to process.
 */
 ...
 *
 * @return An #ti_result value indicating the result of the operation.
 * @retval #TI_OK The children of the current JSON token are skipped successfully.
 * @retval #TI_ERROR_UNEXPECTED_END The end of the JSON document is reached.
 * @retval #TI_ERROR_UNEXPECTED_CHAR An invalid character is detected.
 *
 ...
 * end object or array. For all other token kinds, the reader doesn't move and returns #TI_OK.
 */
