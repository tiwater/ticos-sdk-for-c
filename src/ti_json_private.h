...
#ifndef _ti_JSON_PRIVATE_H
#define _ti_JSON_PRIVATE_H
...
#define _ti_JSON_TOKEN_DEFAULT                     \
  (ti_json_token)                                  \
  {                                                \
    .kind = TI_JSON_TOKEN_NONE, ._internal = { 0 } \
  }
...
enum
{
  ...
  _ti_MAX_JSON_STACK_SIZE = sizeof(((_ti_json_bit_stack*)0)->_internal.ti_json_stack) * 8 // 64
};

enum
{
  // Max size for an already escaped string value (~ half of INT_MAX)
  _ti_MAX_ESCAPED_STRING_SIZE = 1000000000,
  ...
  // still <= 6x).
  _ti_MAX_EXPANSION_FACTOR_WHILE_ESCAPING = 6,

  _ti_MAX_UNESCAPED_STRING_SIZE
  = _ti_MAX_ESCAPED_STRING_SIZE / _ti_MAX_EXPANSION_FACTOR_WHILE_ESCAPING, // 166_666_666 bytes

  // [-][0-9]{16}.[0-9]{15}, i.e. 1+16+1+15 since _ti_MAX_SUPPORTED_FRACTIONAL_DIGITS is 15
  _ti_MAX_SIZE_FOR_WRITING_DOUBLE = 33,
  ...
  _ti_MINIMUM_STRING_CHUNK_SIZE = 64,
  ...
  // multiply by 6). 10 * 6 + 4 = 64, and that fits within _ti_MINIMUM_STRING_CHUNK_SIZE
  _ti_MAX_UNESCAPED_STRING_SIZE_PER_CHUNK = 10,

  // The number of unique values in base 16 (hexadecimal).
  _ti_NUMBER_OF_HEX_VALUES = 16,
};

typedef enum
{
  _ti_JSON_STACK_OBJECT = 1,
  _ti_JSON_STACK_ARRAY = 0,
} _ti_json_stack_item;

TI_INLINE _ti_json_stack_item _ti_json_stack_pop(_ti_json_bit_stack* ref_json_stack)
{
  _ti_PRECONDITION(
      ref_json_stack->_internal.current_depth > 0
      && ref_json_stack->_internal.current_depth <= _ti_MAX_JSON_STACK_SIZE);

  // Don't do the right bit shift if we are at the last bit in the stack.
  if (ref_json_stack->_internal.current_depth != 0)
  {
    ref_json_stack->_internal.ti_json_stack >>= 1U;
  ...
  // true (i.e. 1) means _ti_JSON_STACK_OBJECT, while false (i.e. 0) means _ti_JSON_STACK_ARRAY
  return (ref_json_stack->_internal.ti_json_stack & 1U) != 0 ? _ti_JSON_STACK_OBJECT
                                                             : _ti_JSON_STACK_ARRAY;
}

TI_INLINE void _ti_json_stack_push(_ti_json_bit_stack* ref_json_stack, _ti_json_stack_item item)
{
  _ti_PRECONDITION(
      ref_json_stack->_internal.current_depth >= 0
      && ref_json_stack->_internal.current_depth < _ti_MAX_JSON_STACK_SIZE);
  ...
  ref_json_stack->_internal.current_depth++;
  ref_json_stack->_internal.ti_json_stack <<= 1U;
  ref_json_stack->_internal.ti_json_stack |= (uint32_t)item;
}

TI_NODISCARD TI_INLINE _ti_json_stack_item _ti_json_stack_peek(_ti_json_bit_stack const* json_stack)
{
  _ti_PRECONDITION(
      json_stack->_internal.current_depth >= 0
      && json_stack->_internal.current_depth <= _ti_MAX_JSON_STACK_SIZE);

  // true (i.e. 1) means _ti_JSON_STACK_OBJECT, while false (i.e. 0) means _ti_JSON_STACK_ARRAY
  return (json_stack->_internal.ti_json_stack & 1U) != 0 ? _ti_JSON_STACK_OBJECT
                                                         : _ti_JSON_STACK_ARRAY;
}
