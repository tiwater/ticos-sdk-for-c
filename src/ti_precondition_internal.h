...
/**
 * @file ti_precondition_internal.h
 *
 ...
 *        ti_precondition_failed_fn function signature and then, in your application's
 *        initialization (before calling any Ticos SDK function), call
 *        ti_precondition_failed_set_callback passing it the address of your function.
 *        Now, when any Ticos SDK function detects a precondition failure, it will invoke
 ...
 *
 *        Also, if you define the TI_NO_PRECONDITION_CHECKING symbol when compiling the SDK
 *        code (or adding option -DPRECONDITIONS=OFF with cmake), all of the Ticos SDK
...
#ifndef _ti_PRECONDITION_INTERNAL_H
#define _ti_PRECONDITION_INTERNAL_H
...
// emit warnings for the statements that we put into _ti_PRECONDITION().
...
#if _MSC_VER >= 1920
#define _ti_ANALYSIS_ASSUME(statement) __analysis_assume(statement)
#else
#define _ti_ANALYSIS_ASSUME(statement)
#endif

#ifdef TI_NO_PRECONDITION_CHECKING
#define _ti_PRECONDITION(condition)
#else
#define _ti_PRECONDITION(condition)            \
  do                                           \
  {                                            \
    if (!(condition))                          \
    {                                          \
      ti_precondition_failed_get_callback()(); \
    }                                          \
    _ti_ANALYSIS_ASSUME(condition);            \
  } while (0)
#endif // TI_NO_PRECONDITION_CHECKING

#define _ti_PRECONDITION_RANGE(low, arg, max) _ti_PRECONDITION((low) <= (arg) && (arg) <= (max))

#define _ti_PRECONDITION_NOT_NULL(arg) _ti_PRECONDITION((arg) != NULL)
#define _ti_PRECONDITION_IS_NULL(arg) _ti_PRECONDITION((arg) == NULL)

TI_NODISCARD TI_INLINE bool _ti_span_is_valid(ti_span span, int32_t min_size, bool null_is_valid)
{
  if (min_size < 0)
  {
    return false;
  ...
  uint8_t* const ptr = ti_span_ptr(span);
  int32_t const span_size = ti_span_size(span);
  ...
  */
  ...
  uint8_t* const default_init_ptr = ti_span_ptr((ti_span){ 0 });
  if (null_is_valid)
  {
  ...
  // Example: (ti_span) { .ptr = (uint8_t*)(~0 - 5), .size = 10 } is not a valid span, because most
...
#define _ti_PRECONDITION_VALID_SPAN(span, min_size, null_is_valid) \
  _ti_PRECONDITION(_ti_span_is_valid(span, min_size, null_is_valid))

TI_NODISCARD TI_INLINE bool _ti_span_overlap(ti_span a, ti_span b)
{
  uint8_t* const a_ptr = ti_span_ptr(a);
  uint8_t* const b_ptr = ti_span_ptr(b);

  return a_ptr <= b_ptr ? (a_ptr + ti_span_size(a) > b_ptr) : (b_ptr + ti_span_size(b) > a_ptr);
}

#define _ti_PRECONDITION_NO_OVERLAP_SPANS(a, b) _ti_PRECONDITION(!_ti_span_overlap(a, b))

#include <_ti_cfg_suffix.h>

#endif // _ti_PRECONDITION_INTERNAL_H
