 ...
 * run.
 *
 * To override the default behavior, implement a function matching the #ti_precondition_failed_fn
 * function signature and then, in your application's initialization (before calling any Ticos SDK
 * function), call #ti_precondition_failed_set_callback() passing it the address of your function.
 * Now, when any Ticos SDK function detects a precondition failure, it will invoke your callback
 ...
 *
 * Also, if you define the `TI_NO_PRECONDITION_CHECKING` symbol when compiling the SDK code (or
 * adding option `-DPRECONDITIONS=OFF` with cmake), all of the Ticos SDK precondition checking will
...
#ifndef _ti_PRECONDITION_H
#define _ti_PRECONDITION_H
 ...
 */
typedef void (*ti_precondition_failed_fn)();
 ...
 *
 * Call this function once when your application initializes and before you call and Ticos SDK
 * functions.
 *
 * @param[in] ti_precondition_failed_callback A pointer to the function that will be invoked when an
 * Ticos SDK function detects a precondition failure.
 */
