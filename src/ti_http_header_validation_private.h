...
/**
 * @file ti_http_header_validation_private.h
 *
...
#ifndef _ti_HTTP_HEADER_VALIDATION_PRIVATE_H
#define _ti_HTTP_HEADER_VALIDATION_PRIVATE_H
...
static const uint8_t ti_http_valid_token[256] = {
  0, // 0 ->  null
 ...
 */
TI_NODISCARD TI_INLINE bool ti_http_is_valid_header_name(ti_span name)
{
  uint8_t* name_ptr = ti_span_ptr(name);
  for (int32_t i = 0; i < ti_span_size(name); i++)
  {
    uint8_t c = name_ptr[i];
    if (ti_http_valid_token[c] == 0)
    {
      return false;
    }
  }
...
#endif // _ti_HTTP_HEADER_VALIDATION_PRIVATE_H
