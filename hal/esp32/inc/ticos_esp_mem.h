 ...
 */
void ticos_esp_mem_add_record(void *ptr, int size, const char *tag, int line);
 ...
 * @param line Line number
 */
void ticos_esp_mem_remove_record(void *ptr, const char *tag, int line);

/**
 * @brief Print the all allocation but not released memory
 *
 * @attention Must configure CONFIG_QCLOUD_MEM_DEBUG == y annd esp_log_level_set(ticos_esp_mem, ESP_LOG_INFO);
 */
void ticos_esp_mem_print_record(void);

/**
 * @brief Print memory and free space on the stack
 */
void ticos_esp_mem_print_heap(void);

/**
 * @brief Print the state of tasks in the system
 */
void ticos_esp_mem_print_task(void);
...
#define ticos_esp_MALLOC(size) ({ \
        ...
        if (QCLOUD_MEM_DEBUG) { \
            if(!ptr) { \
                ...
                ticos_esp_mem_add_record(ptr, size, TAG, __LINE__); \
            } \
        } \
        ptr; \
    })
 ...
 *     - NULL when any errors
 */
#define ticos_esp_CALLOC(n, size) ({ \
        ...
        if (QCLOUD_MEM_DEBUG) { \
            if(!ptr) { \
                ...
                ticos_esp_mem_add_record(ptr, (n) * (size), TAG, __LINE__); \
            } \
        } \
        ptr; \
    })
 ...
 *     - NULL when any errors
 */
#define ticos_esp_REALLOC(ptr, size) ({ \
        ...
        if (QCLOUD_MEM_DEBUG) { \
            if(!new_ptr) { \
                ...
                ticos_esp_mem_remove_record(ptr, TAG, __LINE__); \
                ticos_esp_mem_add_record(new_ptr, size, TAG, __LINE__); \
            } \
        } \
        new_ptr; \
    })
 ...
 *     - NULL when any errors
 */
#define ticos_esp_REALLOC_RETRY(ptr, size) ({ \
        void *new_ptr = NULL; \
