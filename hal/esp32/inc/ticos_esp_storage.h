 ...
 *
 * This API is internally called by ticos_esp_init(). Applications may call this
 * only if access to the ESP QCloud storage is required before ticos_esp_init().
 *
 * @return
 *     - ESP_FAIL
 *     - ESP_OK
 */
esp_err_t ticos_esp_storage_init(void);
 ...
 *         esp_err_t ticos_esp_storage_load(const char *key, void *value, size_t *length);
 *         esp_err_t ticos_esp_storage_load(const char *key, void *value, size_t length);
 *
