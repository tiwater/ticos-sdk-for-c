...
#define ticos_esp_LOG_PRINTF(fmt, ...) printf("D [%s, %d]: " fmt, TAG, __LINE__, __VA_ARGS__)
#else
#define ticos_esp_LOG_PRINTF(fmt, ...)
#endif

#define ticos_esp_LOG_CALLOC calloc
#define ticos_esp_LOG_MALLOC malloc
#define ticos_esp_LOG_FREE   free
...
typedef struct {
    ...
    ticos_esp_log_type_t type; /**< Ways to send logs */
    char data[0];               /**< Log data */
} ticos_esp_log_queue_t;
 ...
 *     - ESP_FAIL
 */
esp_err_t ticos_esp_log_get_config(ticos_esp_log_config_t *config);
 ...
 *     - ESP_FAIL
 */
esp_err_t ticos_esp_log_set_config(const ticos_esp_log_config_t *config);
 ...
 *     - ESP_OK
 */
esp_err_t ticos_esp_log_init(const ticos_esp_log_config_t *config);
 ...
 *     - ESP_OK
 */
esp_err_t ticos_esp_log_deinit(void);
 ...
 *     - ESP_FAIL
 */
esp_err_t ticos_esp_log_iothub_write(const char *data, size_t size, esp_log_level_t level, const struct tm *log_time);
 ...
 *      - read_size
 */
esp_err_t ticos_esp_log_flash_read(char *data, size_t *size);
 ...
 *      - size
 */
size_t ticos_esp_log_flash_size(void);
