...
typedef struct ticos_esp_time_config {
    /** If not specified, then 'CONFIG_ticos_esp_SNTP_SERVER_NAME' is used as the SNTP server. */
    char *sntp_server_name;
} ticos_esp_time_config_t;
 ...
 */
#define ticos_esp_ERROR_CHECK(con, err, format, ...) do { \
        if (con) { \
            if(*format != '\0') \
 ...
 *        value rather than a `bool` condition. If the argument of `ticos_esp_ERROR_ASSERT`
 ...
 * @return     [description]
 */
#define ticos_esp_ERROR_ASSERT(err) do { \
        esp_err_t __err_rc = (err); \
        if (__err_rc != ESP_OK) { \
            ESP_LOGW(TAG, "<%s> ticos_esp_ERROR_ASSERT failed, at 0x%08x, expression: %s", \
                     esp_err_to_name(__err_rc), (intptr_t)__builtin_return_address(0) - 3, __ASSERT_FUNC); \
            assert(0 && #err); \
        } \
    } while(0)

#define ticos_esp_ERROR_GOTO(con, lable, format, ...) do { \
        if (con) { \
            if(*format != '\0') \
...
#define ticos_esp_ERROR_CONTINUE(con, format, ...) { \
        if (con) { \
            if(*format != '\0') \
                ESP_LOGW(TAG, format, ##__VA_ARGS__); \
            continue; \
...
#define ticos_esp_ERROR_BREAK(con, format, ...) { \
        if (con) { \
            if(*format != '\0') \
                ESP_LOGW(TAG, format, ##__VA_ARGS__); \
            break; \
...
#define ticos_esp_PARAM_CHECK(con) do { \
        if (!(con)) { \
            ESP_LOGE(TAG, "<ticos_esp_ERR_INVALID_ARG> !(%s)", #con); \
 ...
 */
esp_err_t ticos_esp_reboot(TickType_t wait_ticks);
 ...
 *     - count
 */
int ticos_esp_reboot_unbroken_count(void);
 ...
 *     - count
 */
int ticos_esp_reboot_total_count(void);
 ...
 * @return error on failure
 */
esp_err_t ticos_esp_timesync_start(void);
 ...
 */
bool ticos_esp_timesync_check(void);
 ...
 * @return error on failure
 */
esp_err_t ticos_esp_timesync_wait(uint32_t wait_ms);
 ...
 * 
 */
void ticos_esp_print_system_info(uint32_t interval_ms);
