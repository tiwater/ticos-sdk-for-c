...
static const char *TAG = "ticos_esp_reboot";
static qcloud_reboot_record_t g_reboot_record = {0};

static void ticos_esp_reboot_cb(void *priv)
{
    esp_restart();
}

esp_err_t ticos_esp_reboot(uint32_t wait_ms)
{
...
static void esp_reboot_count_erase_timercb(void *priv)
{
    ...
    ticos_esp_storage_set(REBOOT_RECORD_KEY, &g_reboot_record, sizeof(qcloud_reboot_record_t));

    ESP_LOGD(TAG, "Erase reboot count");
}

static esp_err_t ticos_esp_reboot_unbroken_init()
{
    esp_err_t err          = ESP_OK;
    g_reboot_record.reason = rtc_get_reset_reason(0);

    ticos_esp_storage_init();
    ticos_esp_storage_get(REBOOT_RECORD_KEY, &g_reboot_record, sizeof(qcloud_reboot_record_t));
    ...
    err = ticos_esp_storage_set(REBOOT_RECORD_KEY, &g_reboot_record, sizeof(qcloud_reboot_record_t));
    ticos_esp_ERROR_CHECK(err != ESP_OK, err, "Save the number of reboots within the set time");
    ...
    ticos_esp_ERROR_CHECK(err != ESP_OK, err, "esp_timer_create");

    err = esp_timer_start_once(time_handle, pdMS_TO_TICKS(CONFIG_QCLOUD_REBOOT_UNBROKEN_INTERVAL_TIMEOUT) * 1000U);
    ticos_esp_ERROR_CHECK(err != ESP_OK, err, "esp_timer_start_once");
...
{
    ticos_esp_reboot_unbroken_init();

    if (CONFIG_QCLOUD_REBOOT_UNBROKEN_FALLBACK_COUNT &&
            ticos_esp_reboot_unbroken_count() >= CONFIG_QCLOUD_REBOOT_UNBROKEN_FALLBACK_COUNT) {
...
__attribute((constructor)) static esp_err_t ticos_esp_reboot_unbroken_record()
{
    /**
...
int ticos_esp_reboot_unbroken_count()
{
    return g_reboot_record.unbroken_count;
}

int ticos_esp_reboot_total_count()
{
    return g_reboot_record.total_count;
}

bool ticos_esp_reboot_is_exception(bool erase_coredump)
{
    ...
    ticos_esp_ERROR_CHECK(!coredump_part, false, "<%s> esp_partition_get fail", esp_err_to_name(ret));

    ret = esp_partition_read(coredump_part, 0, &coredump_len, sizeof(ssize_t));
    ticos_esp_ERROR_CHECK(ret, false, "<%s> esp_partition_read fail", esp_err_to_name(ret));
    ...
    if (erase_coredump) {
        ...
        ticos_esp_ERROR_CHECK(ret, false, "<%s> esp_partition_erase_range fail", esp_err_to_name(ret));
    }
