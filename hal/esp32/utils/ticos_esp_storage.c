...
static const char *TAG = "ticos_esp_storage";

esp_err_t ticos_esp_storage_init()
{
...
esp_err_t ticos_esp_storage_erase(const char *key)
{
    ticos_esp_PARAM_CHECK(key);
    ...
    ticos_esp_ERROR_CHECK(ret != ESP_OK, ret, "Open non-volatile storage");
    ...
    ticos_esp_ERROR_CHECK(ret != ESP_OK && ret != ESP_ERR_NVS_NOT_FOUND,
                    ret, "Erase key-value pair, key: %s", key);

    return ESP_OK;
}

esp_err_t ticos_esp_storage_set(const char *key, const void *value, size_t length)
{
    ticos_esp_PARAM_CHECK(key);
    ticos_esp_PARAM_CHECK(value);
    ticos_esp_PARAM_CHECK(length > 0);
    ...
    ticos_esp_ERROR_CHECK(ret != ESP_OK, ret, "Set value for given key, key: %s", key);

    return ESP_OK;
}

esp_err_t ticos_esp_storage_get(const char *key, void *value, size_t length)
{
    ticos_esp_PARAM_CHECK(key);
    ticos_esp_PARAM_CHECK(value);
    ticos_esp_PARAM_CHECK(length > 0);
    ...
    ticos_esp_ERROR_CHECK(ret != ESP_OK, ret, "Get value for given key, key: %s", key);
