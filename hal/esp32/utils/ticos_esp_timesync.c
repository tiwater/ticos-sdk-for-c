...
#define CONFIG_ticos_esp_SNTP_SERVER_NAME "pool.ntp.org"

static bool g_init_done = false;
static const char *TAG = "ticos_esp_timesync";

esp_err_t ticos_esp_timesync_start()
{
    if (sntp_enabled()) {
    ...
    char *sntp_server_name = CONFIG_ticos_esp_SNTP_SERVER_NAME;
...
esp_err_t ticos_esp_timesync_wait(TickType_t ticks_to_wait)
{
    if (!g_init_done) {
        ESP_LOGW(TAG, "Time sync not initialised using 'ticos_esp_timesync_start'");
    }
    ...
    while (ticks_remaining > 0) {
        if (ticos_esp_timesync_check() == true) {
            break;
        }
    ...
    if (ticos_esp_timesync_check() == false) {
