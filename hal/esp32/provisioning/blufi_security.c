...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    ...
    if (blufi_sec == NULL) {
        //ESP_LOGW(TAG, "security","BLUFI Security is not initialized");
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param == NULL) {
            ...
            //ESP_LOGW(TAG,"%s, malloc failed\n", __func__);
            return;
        }
        break;
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            //ESP_LOGW(TAG,"%s, blufi_sec->dh_param == N", __func__);
        ...
        if (ret) {
            //ESP_LOGW(TAG,"%s read param failed %d\n", __func__, ret);
        ...
        if (ret) {
            //ESP_LOGW(TAG,"%s make public failed %d\n", __func__, ret);
