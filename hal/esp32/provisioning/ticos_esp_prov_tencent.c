...
} ticos_esp_prov_event_t;

static const char *TAG = "ticos_esp_prov";
                          ...
                          int32_t event_id, void *event_data)
{
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        if(ticos_esp_prov_ble_is_connected()){
...
void ticos_esp_prov_print_wechat_qr(const char *name, const char *transport)
{
    
}

esp_err_t ticos_esp_prov_data_handler(uint32_t session_id, const uint8_t *inbuf, ssize_t inlen,
                                       uint8_t **outbuf, ssize_t *outlen, void *priv_data)
{
    ...
    ticos_esp_ERROR_GOTO(!token_json, EXIT, "The data format is wrong, the 'token' field is not included");

    g_token = strdup(token_json->valuestring);

    *outlen = asprintf((char **)outbuf,
                       "{\"cmdType\":2,\"productId\":\"%s\",\"deviceName\":\"%s\",\"protoVersion\":\"2.0\"}",
                       ticos_esp_get_product_id(), ticos_esp_get_device_name());
...
static void udp_server_task(void *pvParameters)
{
    ...
    char *rx_buffer   = ticos_esp_MALLOC(UDP_SERVER_BUFFER_MAX_SIZE);
    ...
    ticos_esp_ERROR_GOTO(udp_server_sockfd < 0, EXIT, "Unable to create socket, errno %d, err_str: %s", errno, strerror(errno));

    err = bind(udp_server_sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    ticos_esp_ERROR_GOTO(err < 0, EXIT, "Socket unable to bind: errno %d, err_str: %s", errno, strerror(errno));
    ...
    while (g_prov_server_start_flag) {
        ...
        ticos_esp_ERROR_GOTO(err < 0, EXIT, "recvfrom failed, errno %d, err_str: %s", errno, strerror(errno));
        ticos_esp_ERROR_CONTINUE(err == 0 || !FD_ISSET(udp_server_sockfd, &read_fd), "");
        ...
        ticos_esp_ERROR_GOTO(len < 0, EXIT, "recvfrom failed, errno %d, err_str: %s", errno, strerror(errno));

        ESP_LOGI(TAG, "recvfrom, data: %s", rx_buffer);

        cJSON *json_root = cJSON_Parse(rx_buffer);
        ticos_esp_ERROR_CONTINUE(!json_root, "cJSON_Parse failed, recv data: %.*s", len, rx_buffer);
        ...
        asprintf(&tx_buffer, "{\"cmdType\":%d,\"productId\":\"%s\",\"deviceName\":\"%s\",\"protoVersion\":\"2.0\"}",
                 CMD_DEVICE_REPLY, ticos_esp_get_product_id(), ticos_esp_get_device_name());
        ...
        for (int i = 0; i < 5; i++) {
            ...
            ticos_esp_ERROR_CONTINUE(err < 0, "sendto failed, errno %d, err_str: %s", errno, strerror(errno));
            
            break;
        }

        ticos_esp_FREE(tx_buffer);
...
EXIT:
    ...
    ticos_esp_FREE(rx_buffer);
    vTaskDelete(NULL);
}

esp_err_t ticos_esp_prov_udp_server_start()
{
...
#if (CONFIG_PROVISIONING_SMARTCONFIG) || (CONFIG_PROVISIONING_SOFTAPCONFIG)
...
esp_err_t ticos_esp_prov_udp_server_stop()
{
...
esp_err_t ticos_esp_prov_ble_set_token(char *token, size_t token_len)
{
...
esp_err_t ticos_esp_prov_wait(wifi_config_t *sta_cfg, uint32_t wait_ms)
{
    ...
    if ((bits & QCLOUD_PROV_EVENT_GET_TOKEN) && (bits & QCLOUD_PROV_EVENT_STA_CONNECTED) ) {
        //ticos_esp_storage_set("token", g_token, AUTH_TOKEN_MAX_SIZE);
        esp_wifi_get_config(ESP_IF_WIFI_STA, sta_cfg);
