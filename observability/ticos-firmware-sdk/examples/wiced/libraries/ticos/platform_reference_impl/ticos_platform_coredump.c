...
#ifndef TICOS_PLATFORM_COREDUMP_WICED_DCT_APP_INDEX
#  define TICOS_PLATFORM_COREDUMP_WICED_DCT_APP_INDEX (DCT_OTA_APP_INDEX)
#endif

//! Default storage size to allocate for storing coredump data.
//! Ensure to allocate a bit of extra headroom for metadata.
#ifndef TICOS_PLATFORM_COREDUMP_STORAGE_SIZE_BYTES
#  define TICOS_PLATFORM_COREDUMP_STORAGE_SIZE_BYTES (130 * 1024)
#endif

#ifndef TICOS_PLATFORM_COREDUMP_CAPTURE_STACK_ONLY
#  define TICOS_PLATFORM_COREDUMP_CAPTURE_STACK_ONLY (0)
#endif
...
static wiced_result_t prv_init(void) {
  wiced_app_t app;
  WICED_VERIFY(wiced_framework_app_open(TICOS_PLATFORM_COREDUMP_WICED_DCT_APP_INDEX, &app));

  uint32_t current_size = 0;
  WICED_VERIFY(wiced_framework_app_get_size(&app, &current_size));

  // Resize the SPI flash partition (only if needed):
  if (current_size < TICOS_PLATFORM_COREDUMP_STORAGE_SIZE_BYTES) {
    WICED_VERIFY(wiced_framework_app_set_size(&app, TICOS_PLATFORM_COREDUMP_STORAGE_SIZE_BYTES));
  }
  ...
  if (app_header.count != 1) {
    TICOS_LOG_ERROR("Coredump partition must be contiguous! %d parts found", app_header.count);
    return WICED_ERROR;
  }
  ...
  s_ticos_platform_coredump_ctx = malloc(sizeof(*s_ticos_platform_coredump_ctx));
  if (!s_ticos_platform_coredump_ctx) {
    return WICED_ERROR;
  }
  ...
  if (actual_size < TICOS_PLATFORM_COREDUMP_STORAGE_SIZE_BYTES) {
    TICOS_LOG_ERROR("WICED set size failed? %"PRIu32, actual_size);
    return WICED_ERROR;
  }
  *s_ticos_platform_coredump_ctx = (sTicosPlatformCoredumpCtx) {
      .flash_start = flash_start,
      .flash_end = flash_end,
  };
  // CRC32 to protect it, just in case the data get clobbered:
  s_ticos_platform_coredump_ctx->crc = ticos_platform_crc32(
      s_ticos_platform_coredump_ctx, offsetof(sTicosPlatformCoredumpCtx, crc));
...
static bool prv_get_start_and_end_addr(uint32_t *flash_start, uint32_t *flash_end) {
  const uint32_t actual_crc = ticos_platform_crc32(
      s_ticos_platform_coredump_ctx, offsetof(sTicosPlatformCoredumpCtx, crc));
  if (actual_crc != s_ticos_platform_coredump_ctx->crc) {
    return false;
  }
  *flash_start = s_ticos_platform_coredump_ctx->flash_start;
  *flash_end = s_ticos_platform_coredump_ctx->flash_end;
  return true;
}
