...
//!
//! Example populating ticos_platform_get_device_info() implementation for DA145xx
...
void ticos_platform_get_device_info(sTicosDeviceInfo *info) {
  ...
  if (!s_init) {
    ...
    const size_t build_id_num_chars =
        TICOS_MIN(build_id_chars, sizeof(s_fw_version) - version_len - 1);

    ticos_build_id_get_string(&s_fw_version[version_len], build_id_num_chars);
    s_init = true;
  }
