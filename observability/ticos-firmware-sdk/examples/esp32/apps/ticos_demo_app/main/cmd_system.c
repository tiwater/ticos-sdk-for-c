...
static int test_trace_event_cli_cmd() {
    TICOS_TRACE_EVENT(test);
    // trace event with status code (i.e error code returned from stack)
    TICOS_TRACE_EVENT_WITH_STATUS(test, 0xdeadbeef);
    // trace event with a log of arbitrary data
    TICOS_TRACE_EVENT_WITH_LOG(test, "A trace event with log %d", 1);
    return 0;
}
