...
"""
A script which transforms the ticos-firmware-sdk into a project suitable for inclusion as an arduino library
...
TICOS_RELEASE_PATH = "https://github.com/ticos/ticos-firmware-sdk/archive/refs/tags"


def download_ticos_library(working_dir: str, tag: str):
    ...
    try:
        urllib.request.urlretrieve(
            f"{TICOS_RELEASE_PATH}/{release_artifact_filename}", release_artifact_filepath
        )
    except HTTPError as err:
        if err.code != 404:
            raise
        logging.error("No ticos-firmware-sdk with tag %s exists", tag)
        return None

    return release_artifact_filepath


def extract_ticos_library(working_dir: str, release_artifact_filepath: str):
...
def arduinoify_ticos_sdk(sdk_root_dir: str, result_dir: str, port: str):
    if not os.path.exists(sdk_root_dir):
        raise Exception(f"Ticos SDK directory does not exist: {sdk_root_dir}")
    ...
    for f in os.listdir(port_root):
        ...
        if not os.path.isdir(path_to_rm):
            continue
        logging.debug("Removing %s from ticos-firmware-sdk", path_to_rm)
    ...
    header_regex = re.compile(r'#include\s"ticos/')
    for f in files_to_patch:
        with open(f) as r:
            contents = r.read()
            contents = header_regex.sub(
                '#include "ticos-firmware-sdk/components/include/ticos/', contents
            )
    ...
    shutil.move(sdk_root_dir, os.path.join(library_dir, "src", "ticos-firmware-sdk"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="""
Transforms the ticos-firmware-sdk into a project suitable for inclusion in an arduino library.
...
""",
    ...
    parser.add_argument(
        ...
        "--tag",
        required=False,
        help='The release from https://github.com/ticos/ticos-firmware-sdk/releases to convert to an arduino library (i.e "0.28.2")',
    )
    parser.add_argument(
        "-m",
        "--ticos-sdk",
        required=False,
        help="Path to .tar.gz of the ticos-firmware-sdk to convert to an arduino library",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    if args.tag and args.ticos_sdk:
        raise Exception("--tag and --ticos-sdk can not be used together")

    if not args.tag and not args.ticos_sdk:
        raise Exception("Either --tag or --ticos-sdk must be specified")
    ...
    if args.tag:
        release_artifact_filepath = download_ticos_library(BUILD_DIRECTORY, args.tag)
        extraction_working_dir = BUILD_DIRECTORY
    else:
        release_artifact_filepath = args.ticos_sdk
        extraction_working_dir = os.path.join(BUILD_DIRECTORY, "ticos-firmware-sdk")

    release_artifact_dir = extract_ticos_library(
        extraction_working_dir, release_artifact_filepath
    )

    result_dir = arduinoify_ticos_sdk(release_artifact_dir, args.output, args.port)
