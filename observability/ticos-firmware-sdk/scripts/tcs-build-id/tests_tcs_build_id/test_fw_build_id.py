...
from tcs_build_id import BuildIdInspectorAndPatcher, TicosBuildIdTypes  # noqa isort:skip
...
def test_gnu_build_id_present_but_not_used(capsys, copy_file):
    ...
    result_fixture_filename = os.path.join(ELF_FIXTURES_DIR, "ticos_id_used_gnu_id_present.elf")
    ...
    lines = out.splitlines()
    assert lines == [
        "WARNING: Located a GNU build id but it's not being used by the Ticos SDK",
        "Added Ticos Generated Build ID to ELF: 028aa9800f0524c9b064711925df5ec403df6b16",
        "Found Ticos Build Id: 028aa9800f0524c9b064711925df5ec403df6b16",
    ]


def test_ticos_id_unpopulated(capsys, copy_file):
    elf_fixture_filename = os.path.join(
        ELF_FIXTURES_DIR, "ticos_build_id_present_and_unpopulated.elf"
    )
    result_fixture_filename = os.path.join(
        ELF_FIXTURES_DIR, "ticos_build_id_present_and_populated.elf"
    )
    ...
    lines = out.splitlines()
    assert lines == [
        "Added Ticos Generated Build ID to ELF: 16e0fe39af176cfa4cf961321ccf5193c2590451",
        "Found Ticos Build Id: 16e0fe39af176cfa4cf961321ccf5193c2590451",
    ]


def test_ticos_id_populated(capsys, copy_file):
    elf_fixture_filename = os.path.join(
        ELF_FIXTURES_DIR, "ticos_build_id_present_and_populated.elf"
    )
    ...
    out, _ = capsys.readouterr()
    lines = out.splitlines()
    assert lines == ["Found Ticos Build Id: 16e0fe39af176cfa4cf961321ccf5193c2590451"]


def test_no_ticos_sdk_present():
    elf_fixture_filename = os.path.join(ELF_FIXTURES_DIR, "no_ticos_symbols.elf")
    with open(elf_fixture_filename, "rb") as elf_fixture_file:
        b = BuildIdInspectorAndPatcher(elf_fixture_file)
        with pytest.raises(
            Exception, match="Could not locate 'g_ticos_build_id' symbol in provided ELF"
        ):
            b.check_or_update_build_id()
...
@pytest.mark.parametrize(
    ...
    [
        (
            "ticos_build_id_present_and_populated.elf",
            (
                TicosBuildIdTypes.TICOS_BUILD_ID_SHA1,
                "16e0fe39af176cfa4cf961321ccf5193c2590451",
                None,
            ),
        ),
        (
            "no_ticos_symbols.elf",
            # BuildIdException is caught and None, None is returned:
            (None, None, None),
        ),
        (
            "ticos_build_id_with_short_len.elf",
            (
                TicosBuildIdTypes.TICOS_BUILD_ID_SHA1,
        ...
        (
            ...
            (
                TicosBuildIdTypes.GNU_BUILD_ID_SHA1,
