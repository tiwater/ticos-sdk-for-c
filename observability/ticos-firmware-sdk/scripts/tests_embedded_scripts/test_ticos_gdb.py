...
from ticos_gdb import (  # noqa: E402 M900
...
# We save the time in the coredump generated so each run of "ticos coredump" looks unique
# Let's monkeypatch time so we get a reproducible time in out unit tests
@pytest.fixture(autouse=True)
def _patch_datetime_now(mocker):
    mocker.patch("ticos_gdb.time", return_value=float(1565034658.6))
...
def http_expect_request():
    ...
    with mock.patch("ticos_gdb.HTTPSConnection", _create_connection_constructor("https")):
        with mock.patch("ticos_gdb.HTTPConnection", _create_connection_constructor("http")):
            yield _http_expect_request

    assert actual_request_count == len(expected_requests)


TEST_EMAIL = "martijn@ticos.com"
...
def test_config(mocker, tmp_path):
    """Updates TICOS_CONFIG to contain TEST_... values"""
    rv = TicosConfig()
    mocker.patch("ticos_gdb.TICOS_CONFIG", rv)
...
def test_http_basic_auth():
    ...
    headers = add_basic_auth("martijn@ticos.com", "open_sesame", {"Foo": "Bar"})
    assert headers == {
        "Foo": "Bar",
...
def test_coredump_command_with_project_key(http_expect_request, test_config):
    ...
    test_config.ingress_uri = "https://custom-ingress.ticos.com"

    http_expect_request(
        "https://custom-ingress.ticos.com/api/v0/upload/coredump",
        "POST",
        ANY,  # Not checking the request body at the moment
        {"Content-Type": "application/octet-stream", "Ticos-Project-Key": TEST_PROJECT_KEY},
        200,
        {},
    )
...
def test_coredump_command_with_login_no_existing_release_or_symbols(
    ...
    http_expect_request(
        "https://api.ticos.com/api/v0/organizations/acme-inc/projects/smart-sink/software_types/main/software_versions/1.0.0-md5+b61b2d6c",
        "GET",
        ANY,
        ANY,
        404,
    ...
    # Upload Symbols
    token = "token-foo"
    upload_url = "https://ticos-test-east1.s3.amazonaws.com/symbols/foo"
    http_expect_request(
        "https://api.ticos.com/api/v0/organizations/acme-inc/projects/smart-sink/upload",
        "POST",
        ANY,
        ANY,
        200,
    ...
    http_expect_request(
        "https://api.ticos.com/api/v0/organizations/acme-inc/projects/smart-sink/symbols",
        "POST",
    ...
    # Get Project-Key:
    http_expect_request(
        "https://api.ticos.com/api/v0/organizations/acme-inc/projects/smart-sink/api_key",
        "GET",
        ANY,
        ANY,
        200,
    ...
    http_expect_request(
        "https://ingress.ticos.com/api/v0/upload/coredump",
        "POST",
        _check_request_body,
        {"Content-Type": "application/octet-stream", "Ticos-Project-Key": TEST_PROJECT_KEY},
        200,
...
https://app.ticos.com/organizations/acme-inc/projects/smart-sink/issues?live"""
        in stdout
    )
...
def test_login_command_simple(http_expect_request, test_config):
    ...
    http_expect_request(
        "https://api.ticos.com/auth/me", "GET", None, TEST_AUTH_HEADERS, 200, {"id": 123}
    )
...
def test_login_command_with_all_options(http_expect_request, test_config):
    ...
    test_api_uri = "http://dev-api.ticos.com:8000"
    test_ingress_uri = "http://dev-ingress.ticos.com"

    http_expect_request(
        "http://dev-api.ticos.com:8000/auth/me", "GET", None, TEST_AUTH_HEADERS, 200, {"id": 123}
    )
...
def test_has_uploaded_symbols(
    ...
    http_expect_request(
        "https://api.ticos.com/api/v0/organizations/{}/projects/{}/software_types/{}/software_versions/{}".format(
...
def test_post_chunk_data(http_expect_request):
    ...
    base_uri = "https://example.chunks.ticos.com"
