...
"""
A script which can be used to add the ticos-firmware-sdk to a project using an Eclipse-based IDE
"""
...
def get_depth_from_parent(project_dir, ticos_dir):
    common_prefix = os.path.commonprefix([ticos_dir, project_dir])
    depth = 1
    dirname = project_dir

    # some projects are in the root of the project dir- if the ticos dir is
    ...
    # parent for the project and ticos dirs
    while True:
...
def files_to_link(dir_glob, virtual_dir, common_prefix, parent_dir):
    ...
    for file_name in files:
        ...
        #  - skip adding "ticos_demo_http" from demo component
...
def patch_project(
    ...
    if not os.path.isdir(ticos_sdk_dir) or not os.path.isfile(
        "{}/VERSION".format(ticos_sdk_dir)
    ):
        raise Exception("Could not locate ticos-firmware-sdk at {}".format(ticos_sdk_dir))

    if location_prefix is None:
        # No prefix was given so paths will be generated relative to project root
        common_prefix, depth = get_depth_from_parent(project_dir, ticos_sdk_dir)
        parent_dir = "PARENT-{}-PROJECT_LOC".format(depth)
    else:
        common_prefix = ticos_sdk_dir
        relative_path = os.path.relpath(
            ticos_sdk_dir,
            os.path.commonprefix([ticos_sdk_dir, location_prefix[1]]),
        )
    ...
    logging.debug("Ticos Firmware SDK Path: %s", ticos_sdk_dir)
    logging.debug("Eclipse Ticos Root:      %s", parent_dir)
    ...
    # We want this script to be idempotent so remove any "ticos_" sources already
    # added. We will just be adding them back below.
    for link in linked_resources.findall("link"):
        name = link.find(".//name")
        if name is not None and "ticos_" in name.text:
            linked_resources.remove(link)

    comp_folder_name = "ticos_components"
    ...
    for component in components:
        for ele in files_to_link(
            dir_glob="{}/components/{}/**/*.c".format(ticos_sdk_dir, component),
    ...
    if target_port is not None:
        ...
        port_folder_name = "ticos_{}".format(port_folder_name)
        ...
        for ele in files_to_link(
            dir_glob="{}/ports/{}/*.c".format(ticos_sdk_dir, target_port),
        ...
        if target_port == "dialog/da1469x":
            for ele in files_to_link(
                dir_glob="{}/ports/freertos/**/*.c".format(ticos_sdk_dir),
                virtual_dir=port_folder_name,
                common_prefix=common_prefix,
                parent_dir=parent_dir,
            ):
...
def patch_cproject(
    ...
    #
    # Add required Ticos include paths into build for all build configurations:
    #   ${TICOS_FIRMWARE_SDK}/components/include
    #   ${TICOS_FIRMWARE_SDK}/ports/include
    #
    ...
    ticos_sdk_include_paths = [
        "${workspace_loc:/${ProjName}/ticos_includes/components/include}",
        "${workspace_loc:/${ProjName}/ticos_includes/ports/include}",
    ]
    ...
    for include_option in include_options:
        ...
        for include_path in list_option_values:
            ...
            if "ticos_includes" in path:
                include_option.remove(include_path)

        for path in ticos_sdk_include_paths:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="""
Patches an Eclipse .project file to include sources for ticos-firmware-sdk.

Example Usage:

# cd into directory with .project file
$ python eclipse_patch.py --project-dir . --ticos-sdk-dir /path/to/ticos-firmware-sdk
""",
    )
    parser.add_argument(
        "-p",
    ...
    parser.add_argument(
        ...
        "--ticos-sdk-dir",
        required=True,
        help="The directory ticos-firmware-sdk was copied to",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--location-prefix",
        help=(
            "The default behavior will add ticos-firmware-sdk files to the eclipse project using"
    ...
    ticos_sdk_dir = os.path.realpath(args.ticos_sdk_dir)
