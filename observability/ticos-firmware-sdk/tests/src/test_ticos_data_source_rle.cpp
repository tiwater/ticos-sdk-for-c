...
static const sTicosDataSourceImpl s_test_data_source = {
  .has_more_msgs_cb = prv_has_msgs,
  .read_msg_cb = prv_read_msg_data,
  .mark_msg_read_cb = prv_mark_msg_read,
};

TEST_GROUP(TicosDataSourceRle){
  void setup() {
    s_active_data = NULL;
    s_active_data_size = 0;
    ticos_data_source_rle_encoder_set_active(&s_test_data_source);
  }
  void teardown() {
    ticos_data_source_rle_mark_msg_read();
  }
};

TEST(TicosDataSourceRle, Test_DataSourceHasMoreMsgs) {
  const uint8_t fake_core[] = { 1, 1 };
  s_active_data = &fake_core[0];
  s_active_data_size = sizeof(fake_core);

  size_t total_size = 0;
  bool more_msgs = ticos_data_source_rle_has_more_msgs(&total_size);
  CHECK(more_msgs);
  ...
  s_active_data = NULL;
  total_size = 0;
  more_msgs = ticos_data_source_rle_has_more_msgs(&total_size);
  CHECK(more_msgs);
  LONGS_EQUAL(2, total_size);
...
static void prv_get_coredump_data(uint8_t *buf, size_t buf_len, size_t fill_call_size) {
  for (size_t i = 0; i < buf_len; i += fill_call_size) {
    const size_t bytes_to_read = TICOS_MIN(fill_call_size, buf_len - i);
    bool success = ticos_data_source_rle_read_msg(i, &buf[i], bytes_to_read);
    CHECK(success);
  }
}
...
static void prv_check_pattern(const uint8_t *in, size_t in_len,
  ...
  for (size_t fill_size = 1; fill_size < expected_out_len; fill_size++) {
    ...
    size_t total_size = 0;
    bool more_msg = ticos_data_source_rle_has_more_msgs(&total_size);
    CHECK(more_msg);
...
TEST(TicosDataSourceRle, Test_DataSourceEndsWithRepeat) {
...
TEST(TicosDataSourceRle, Test_DataSourceMultiByteVarintAndEndsWithNonRepeat) {
