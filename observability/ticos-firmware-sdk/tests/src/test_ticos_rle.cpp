...
static void prv_update_result_buf(sTicosRleResultCtx *result_ctx,
                                  const sTicosRleWriteInfo *write_info) {
...
static void prv_encode_with_fill_interval(
    sTicosRleResultCtx *result_ctx, size_t total_size, size_t fill_call_size) {
  sTicosRleCtx ctx = { 0 };

  ticos_rle_encode(&ctx, NULL, 0); // should be a no-op
  for (size_t i = 0; i < result_ctx->orig_buf_len;) {
    const size_t read_len = TICOS_MIN(fill_call_size, result_ctx->orig_buf_len - i);
    const size_t bytes_encoded = ticos_rle_encode(&ctx, &result_ctx->orig_buf[i], read_len);
    prv_update_result_buf(result_ctx, &ctx.write_info);
    i += bytes_encoded;
  }
  ticos_rle_encode_finalize(&ctx);
  prv_update_result_buf(result_ctx, &ctx.write_info);
  LONGS_EQUAL(total_size, ctx.total_rle_size);
}

static void prv_encode(sTicosRleResultCtx *result_ctx, size_t total_size) {
...
static void prv_check_pattern(const uint8_t *in, size_t in_len,
  ...
  sTicosRleResultCtx result_ctx = { 0 };
...
TEST(TicosRle, Test_QuickTransitions) {
...
TEST(TicosRle, Test_2and3RunSequences) {
  const uint8_t pattern[] = {
    0x1,
...
TEST(TicosRle, Test_EndWithRepeatSequence) {
...
TEST(TicosRle, Test_EndWithNonRepeat) {
...
TEST(TicosRle, Test_VariousEncoderSizes) {
