...
static char *prv_setup_encoder(sTicosCborEncoder *encoder, size_t expected_size) {
  char *buf = (char *)calloc(expected_size, 1);
  ticos_cbor_encoder_init(encoder, prv_write_cb, buf, expected_size);
  return buf;
}

static void prv_check_result(sTicosCborEncoder *encoder, char *result,
...
TEST(TcsCompactLog, Test_TcsCompactLog_NoArg) {
  ...
  ticos_log_compact_serialize(&encoder, prv_get_fake_log_id(), compressed_fmt);
...
TEST(TcsCompactLog, Test_TcsCompactLog_Int32) {
  ...
  const bool success = ticos_log_compact_serialize(&encoder, prv_get_fake_log_id(), compressed_fmt, 11);
  CHECK(success);
  prv_check_result(&encoder, result_buf, expected_seq, expected_seq_len);
}
...
TEST(TcsCompactLog, Test_TcsCompactLog_Int64) {
  ...
  const bool success = ticos_log_compact_serialize(&encoder, prv_get_fake_log_id(), compressed_fmt, bigint);
  CHECK(success);
  prv_check_result(&encoder, result_buf, expected_seq, expected_seq_len);
}
...
TEST(TcsCompactLog, Test_TcsCompactLog_Double) {
  ...
  const bool success = ticos_log_compact_serialize(&encoder, prv_get_fake_log_id(), compressed_fmt, a_double);
  CHECK(success);
  prv_check_result(&encoder, result_buf, expected_seq, expected_seq_len);
}
...
TEST(TcsCompactLog, Test_TcsCompactLog_String) {
  ...
  const bool success = ticos_log_compact_serialize(&encoder, prv_get_fake_log_id(), compressed_fmt, str_arg);
  CHECK(success);
  prv_check_result(&encoder, result_buf, expected_seq, expected_seq_len);
}
...
TEST(TcsCompactLog, Test_TcsCompactLog_MultiArg) {
  ...
  for (size_t i = 0; i <= expected_seq_len; i++) {
    ...
    const char *str_arg = "hello";
    const bool success = ticos_log_compact_serialize(&encoder, prv_get_fake_log_id(),
                                                compressed_fmt,
                                                hhx, lld, g, str_arg);
