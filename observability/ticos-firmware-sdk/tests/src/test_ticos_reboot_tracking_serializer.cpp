...
static void prv_run_reset_reason_serializer_test(const void *expected_data,
  ...
  fake_ticos_event_storage_set_available_space(expected_data_len);

  mock().expectOneCall("prv_begin_write");
  mock().expectOneCall("prv_finish_write").withParameter("rollback", false);
  mock().expectOneCall("ticos_reboot_tracking_clear_reset_info");
  const int rv = ticos_reboot_tracking_collect_reset_info(s_fake_event_storage_impl);
  LONGS_EQUAL(0, rv);
