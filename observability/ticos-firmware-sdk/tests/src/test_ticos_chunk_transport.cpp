...
TEST_GROUP(TicosChunkTransport){
  void setup() {
    ...
    // restore defaults
    s_chunk_ctx.total_size = TICOS_ARRAY_SIZE(s_test_msg);
...
static void prv_check_chunk(sTcsChunkTransportCtx *ctx, bool expect_md, size_t receive_buf_len,
  ...
  bool md = ticos_chunk_transport_get_next_chunk(ctx, &actual_chunk[0], &receive_buf_len);
...
TEST(TicosChunkTransport, Test_ChunkerMultiPart) {
...
TEST(TicosChunkTransport, Test_ChunkerMultiPartLastMessageJustCrc) {
  ...
  receive_buf_size = TICOS_MIN_CHUNK_BUF_LEN;
  prv_check_chunk(&s_chunk_ctx, !md, receive_buf_size, &expected_msg_3, sizeof(expected_msg_3));
}


TEST(TicosChunkTransport, Test_ChunkerMultiCallChunkLastMessageJustCrc) {
...
TEST(TicosChunkTransport, Test_BufTooSmall) {
  size_t receive_buf_len = 8;
  uint8_t receive_buf[receive_buf_len];
  bool md = ticos_chunk_transport_get_next_chunk(&s_chunk_ctx, &receive_buf[0], &receive_buf_len);
  CHECK(md);
  LONGS_EQUAL(0, receive_buf_len);
}

TEST(TicosChunkTransport, Test_ChunkerSingleMsg) {
  const bool md = true;
...
TEST(TicosChunkTransport, Test_ChunkerSingleMsgOversizeBuffer) {
  const uint8_t expected_msg_all[] = { 0x08, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0xa, 0x1b, 0x13 };
  ...
  const bool md = ticos_chunk_transport_get_next_chunk(&s_chunk_ctx, &actual_chunk[0], &buf_len);
  CHECK(!md);
...
TEST(TicosChunkTransport, Test_ChunkerBigMessage) {
  uint8_t s_big_msg[4072] = { 0x1, 0x2, 0x3 };
  s_active_msg = &s_big_msg[0];

  s_chunk_ctx.total_size = TICOS_ARRAY_SIZE(s_big_msg);

  // get info before sending the message
  ticos_chunk_transport_get_chunk_info(&s_chunk_ctx);
  LONGS_EQUAL(s_chunk_ctx.total_size + 1 + 2, s_chunk_ctx.single_chunk_message_length);
  ...
  bool md_avail = ticos_chunk_transport_get_next_chunk(&s_chunk_ctx, &read_buffer[0], &read_buffer_size);
...
TEST(TicosChunkTransport, Test_ChunkerSingleMsgMultiChunkEnabled) {
  s_chunk_ctx.enable_multi_call_chunk = true;
...
TEST(TicosChunkTransport, Test_ChunkerMultiCallSingleChunk) {
  uint8_t s_big_msg[4072] = { 0x1, 0x2, 0x3 };
  s_active_msg = &s_big_msg[0];
