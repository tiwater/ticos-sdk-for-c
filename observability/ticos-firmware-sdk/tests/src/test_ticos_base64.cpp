...
static void prv_test_basic_encoder(const char *input_str, const char *expected_out_str) {
  ...
  const size_t encode_len = TICOS_BASE64_ENCODE_LEN(in_len);
  char result[encode_len + 1];
  memset(result, 0xA5, sizeof(result));
  result[encode_len] = '\0';
  ticos_base64_encode(input_str, in_len, result);
...
static void prv_test_inplace_encoder(const char *input_str, const char *expected_out_str) {
  ...
  ticos_base64_encode_inplace(in_out_buf, in_len);
...
TEST(TicosMinimalCbor, Test_RfcTestVectors) {
  prv_run_test_cases("", "");
...
TEST(TicosMinimalCbor, Test_All_BasicEncoder) {
  uint8_t bin_in[256];
  ...
  const size_t encode_len = TICOS_BASE64_ENCODE_LEN(bin_len);
  ...
  ticos_base64_encode(bin_in, bin_len, result);
  STRCMP_EQUAL(expected_out_str, result);
}

TEST(TicosMinimalCbor, Test_All_InPlace) {
  const size_t bin_len = 256;
  uint8_t in_out_buf[TICOS_BASE64_ENCODE_LEN(bin_len) + 1];
  ...
  const char *expected_out_str =
      ...
      "xsfIycrLzM3Oz9DR0tPU1dbX2Nna29zd3t/g4eLj5OXm5+jp6uvs7"
      "e7v8PHy8/T19vf4+fr7/P3+/w==";
  ticos_base64_encode_inplace(in_out_buf, bin_len);
  MEMCMP_EQUAL(expected_out_str, in_out_buf, encode_len);
}

TEST(TicosMinimalCbor, Test_ConversionMacros) {
  // multiples of 3 should always match

  size_t encode_len = TICOS_BASE64_ENCODE_LEN(3);
  LONGS_EQUAL(3, TICOS_BASE64_MAX_DECODE_LEN(encode_len));

  encode_len = TICOS_BASE64_ENCODE_LEN(300);
  LONGS_EQUAL(300, TICOS_BASE64_MAX_DECODE_LEN(encode_len));

  // for non multiples of 3, there will be up to 2 padding characters

  encode_len = TICOS_BASE64_ENCODE_LEN(4);
  LONGS_EQUAL(6, TICOS_BASE64_MAX_DECODE_LEN(encode_len));

  encode_len = TICOS_BASE64_ENCODE_LEN(5);
  LONGS_EQUAL(6, TICOS_BASE64_MAX_DECODE_LEN(encode_len));
}
