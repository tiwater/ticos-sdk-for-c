...
extern "C" {
  #include "fakes/fake_ticos_platform_coredump_storage.h"
  ...
  bool ticos_coredump_read(uint32_t offset, void *buf, size_t buf_len) {
    ...
    return fake_ticos_platform_coredump_storage_read(offset, buf, buf_len);
  }

  bool ticos_platform_coredump_storage_read(uint32_t offset, void *buf,
                                               size_t buf_len) {
    mock().actualCall(__func__);
    return fake_ticos_platform_coredump_storage_read(offset, buf, buf_len);
  }
  ...
  bool ticos_build_info_read(sTicosBuildInfo *info) {
    ...
    if (build_info_available) {
      memcpy(info->build_id, s_fake_ticos_build_id, sizeof(s_fake_ticos_build_id));
    }
...
TEST_GROUP(TcsCoredumpTestGroup) {
  void setup() {
    ...
    ticos_platform_coredump_storage_erase(0, sizeof(s_storage_buf));
...
static bool prv_check_coredump_validity_and_get_size(size_t *total_coredump_size) {
  // NB: This should only be called while the system is running so ticos_coredump_read
  // should always be used. Let's use a mock to verify that.
  mock().expectOneCall("ticos_coredump_read");
  const bool has_coredump = ticos_coredump_has_valid_coredump(total_coredump_size);
  mock().checkExpectations();
  return has_coredump;
}
...
static bool prv_collect_regions_and_save(void *regs, size_t size,
  ...
  if (num_regions != 0) {
    ...
    mock().expectOneCall("ticos_platform_coredump_storage_read");
    mock().ignoreOtherCalls(); // We will test handling ticos_build_info_read() explicitly
  }

  sTicosCoredumpSaveInfo info = {
    .regs = regs,
    .regs_size = size,
    .trace_reason = (eTicosRebootReason)trace_reason,
    .regions = regions,
    .num_regions = num_regions,
  };
  const bool success = ticos_coredump_save(&info);
  mock().checkExpectations();
  return success;
}
...
static size_t prv_compute_space_needed_with_build_id(void *regs, size_t size, uint32_t trace_reason,
  ...
  // medium itself
  mock().expectNCalls(0, "ticos_coredump_read");
  mock().expectNCalls(0, "ticos_platform_coredump_storage_read");
  ...
  const size_t size_needed = ticos_coredump_get_save_size(&info);
  mock().checkExpectations();
  mock().clear();
  return size_needed;
}
...
TEST(TcsCoredumpTestGroup, Test_TcsCoredumpEmptyStorage) {
  ...
  size_t total_size;
  const bool has_coredump = ticos_coredump_has_valid_coredump(&total_size);
  CHECK(!has_coredump);
}
...
TEST(TcsCoredumpTestGroup, Test_TcsCoredumpStorageTooSmall) {
  ...
  for (size_t i = 1; i <= coredump_size_with_build_id; i++) {
    ...
    fake_ticos_platform_coredump_storage_setup(storage, i, i);
    ...
    if (i >= build_id_start_offset) {
      mock().expectOneCall("ticos_build_info_read").andReturnValue(do_collect_build_id);
    }
...
TEST(TcsCoredumpTestGroup, Test_TcsCoredumpTruncated) {
  ...
  fake_ticos_platform_coredump_storage_setup(NULL, 0, 0);
  ...
  fake_ticos_platform_coredump_storage_setup(storage, storage_size, storage_size);
  bool success = prv_collect_regions_and_save((void *)&regs, sizeof(regs), trace_reason);
  CHECK(success);
  ...
  for (size_t i = 0; i < block_hdr_len + 3; i++) {
    fake_ticos_platform_coredump_storage_setup(storage, storage_size + i, storage_size + i);
...
TEST(TcsCoredumpTestGroup, Test_CoredumpReadHeaderMagic) {
  ...
  uint32_t word = 0;
  const bool success = fake_ticos_platform_coredump_storage_read(0, &word, sizeof(word));
  CHECK(success);
...
TEST(TcsCoredumpTestGroup, Test_TcsCoredumpSaveCore) {
  ...
  mock().expectOneCall("ticos_build_info_read").andReturnValue(true);
  ...
  total_length += (6 * segment_hdr_sz) + sizeof(s_fake_ticos_build_id) + strlen(info.device_serial) +
  ...
  coredump_buf += segment_hdr_sz;
  MEMCMP_EQUAL(s_fake_ticos_build_id, coredump_buf, sizeof(s_fake_ticos_build_id));
  coredump_buf += sizeof(s_fake_ticos_build_id);
  ...
  // should have been no calls to ticos_coredump_read
  mock().checkExpectations();
