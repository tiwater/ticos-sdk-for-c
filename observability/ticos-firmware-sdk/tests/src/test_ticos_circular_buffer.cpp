...
TEST(TcsCircularBufferTestGroup, Test_TcsCircularBufferInit) {
  ...
  success = ticos_circular_buffer_init(&buffer, storage_buf, 0);
  CHECK(!success);

  success = ticos_circular_buffer_init(&buffer, storage_buf, sizeof(storage_buf));
  CHECK(success);

  size_t bytes_to_read = ticos_circular_buffer_get_read_size(&buffer);
  LONGS_EQUAL(0, bytes_to_read);

  bytes_to_read = ticos_circular_buffer_get_read_size(NULL);
  LONGS_EQUAL(0, bytes_to_read);

  size_t space_available = ticos_circular_buffer_get_write_size(&buffer);
  LONGS_EQUAL(sizeof(storage_buf), space_available);

  space_available = ticos_circular_buffer_get_write_size(NULL);
...
TEST(TcsCircularBufferTestGroup, Test_TcsCircularWriteAtOffsetBasic) {
  ...
  success = ticos_circular_buffer_write_at_offset(&buffer, 0, seq1, sizeof(seq1));
  CHECK(success);

  // overwrite first two bytes and fill rest of buffer
  const uint8_t seq2[] = { 0x3, 0x4, 0x5, 0x6 };
  success = ticos_circular_buffer_write_at_offset(&buffer, 2, seq2, sizeof(seq2));
  CHECK(success);

  // buffer should be full
  uint8_t result[buffer_size];
  memset(result, 0x0, sizeof(result));
  success = ticos_circular_buffer_read(&buffer, 0, &result, sizeof(result));
  CHECK(success);
...
TEST(TcsCircularBufferTestGroup, Test_TcsCircularWriteAndReadBasic) {
  ...
  success = ticos_circular_buffer_read(&buffer, 0, bytes_read, sizeof(bytes_read));
  CHECK(!success);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    success = ticos_circular_buffer_write(NULL, bytes_to_write, sizeof(bytes_to_write));
    CHECK(!success);
    success = ticos_circular_buffer_write(&buffer, NULL, 0);
    CHECK(!success);

    success = ticos_circular_buffer_write(&buffer, bytes_to_write, sizeof(bytes_to_write));
    // 2nd iteration should fail because we are out of space
    LONGS_EQUAL(i == 0, success);
    bytes_to_read = ticos_circular_buffer_get_read_size(&buffer);
    LONGS_EQUAL(sizeof(bytes_to_write), bytes_to_read);

    space_available = ticos_circular_buffer_get_write_size(&buffer);
  ...
  success = ticos_circular_buffer_write_at_offset(&buffer, sizeof(bytes_to_write) + 1,
                                                     bytes_to_write, sizeof(bytes_to_write));
  CHECK(!success);


  // trying a read with a NULL buffer should fail
  success = ticos_circular_buffer_read(NULL, 0, bytes_read, sizeof(bytes_read));
  CHECK(!success);
  success = ticos_circular_buffer_read(&buffer, 0, NULL, 0);
  CHECK(!success);

  success = ticos_circular_buffer_read(&buffer, 0, bytes_read, sizeof(bytes_to_write));
  CHECK(success);
  MEMCMP_EQUAL(bytes_to_write, bytes_read, sizeof(bytes_to_write));

  // trying to consume too many bytes should fail
  success = ticos_circular_buffer_consume(&buffer, sizeof(bytes_read));
  CHECK(!success);
  success = ticos_circular_buffer_consume_from_end(&buffer, sizeof(bytes_read));
  CHECK(!success);

  // trying to consume without a buffer should fail
  success = ticos_circular_buffer_consume(NULL, sizeof(bytes_to_write));
  CHECK(!success);
  success = ticos_circular_buffer_consume_from_end(NULL, sizeof(bytes_to_write));
  CHECK(!success);

  success = ticos_circular_buffer_consume(&buffer, sizeof(bytes_to_write));
  CHECK(success);
...
static void prv_read_buf_and_check(sTcsCircularBuffer *buffer, uint8_t start_idx,
  ...
  for (uint8_t i = 0; i < sizeof(read_buf); i++) {
    success = ticos_circular_buffer_read(buffer, i, &read_buf[i], 1);
    CHECK(success);
  ...
  // as one big read
  memset(read_buf, 0x0, sizeof(read_buf));
  success = ticos_circular_buffer_read(buffer, 0, read_buf, sizeof(read_buf));
  CHECK(success);
  ...
  success = ticos_circular_buffer_consume(buffer, total_len);
  CHECK(success);
}
...
TEST(TcsCircularBufferTestGroup, Test_TcsCircularWriteAndReadWrapAround) {
  ...
  for (size_t i = 0; i < 99; i++) {
    ...
    success = ticos_circular_buffer_write(&buffer, init_pattern_buf,
                                             sizeof(init_pattern_buf));
    CHECK(success);

    // re-write first byte
    success = ticos_circular_buffer_write_at_offset(&buffer, sizeof(write_buf), write_buf, 1);
    CHECK(success);

    // consume one byte
    success = ticos_circular_buffer_consume_from_end(&buffer, 1);
    CHECK(success);

    // re-write remaining
    success = ticos_circular_buffer_write_at_offset(&buffer, sizeof(write_buf) - 2, &write_buf[1], sizeof(write_buf) - 1);
    CHECK(success);
...
TEST(TcsCircularBufferTestGroup, Test_TcsCircularWriteAndGetReadPointerBadInput) {
  ...
  uint8_t data = 0xab;
  success = ticos_circular_buffer_write(&buffer, &data, sizeof(data));
  CHECK(success);

  uint8_t *read_ptr;
  size_t read_ptr_len;
  success = ticos_circular_buffer_get_read_pointer(NULL, 0, &read_ptr, &read_ptr_len);
  CHECK(!success);

  success = ticos_circular_buffer_get_read_pointer(&buffer, 0, NULL, &read_ptr_len);
  CHECK(!success);

  success = ticos_circular_buffer_get_read_pointer(&buffer, 0, &read_ptr, NULL);
  CHECK(!success);

  success = ticos_circular_buffer_get_read_pointer(&buffer, 2, &read_ptr, &read_ptr_len);
  CHECK(!success);
}
...
TEST(TcsCircularBufferTestGroup, Test_TcsCircularWriteAndGetReadPointer) {
  ...
  for (uint8_t i = 0; i < buffer_size; i++) {
    ...
    uint8_t *read_ptr;
    size_t read_ptr_len;
    success = ticos_circular_buffer_get_read_pointer(&buffer, 0, &read_ptr, &read_ptr_len);
    CHECK(success);
  ...
  for (uint8_t i = 0; i < buffer_size; i++) {
    success = ticos_circular_buffer_consume(&buffer, 1);
    CHECK(success);
