...
#if TICOS_DATA_SOURCE_RLE_ENABLED
...
//! @return true when the check has completed and calling ticos_data_source_rle_has_more_msgs()
...
//! @return the offset the next call to ticos_data_source_rle_read_msg() will start reading from
...
//! Helper function that builds buffer returned from ticos_data_source_rle_read_msg()
//!
...
static const sTicosDataSourceImpl *s_active_data_source = NULL;

typedef enum {
  kTicosDataSourceRleState_Inactive = 0,
  // Searching for the next sequence to encode
  kTicosDataSourceRleState_FindingSeqLength,
  // A sequence to encode has been found and is being written out
  // via calls to ticos_data_source_rle_read_msg()
  kTicosDataSourceRleState_WritingSequence,
} eTicosDataSourceRleState;

typedef struct {
  eTicosDataSourceRleState state;
...
} sTicosDataSourceRleEncodeCtx;

typedef struct {
  sTicosDataSourceImpl *data_source;
  size_t original_size;
  size_t total_rle_size;
  sTicosRleCtx rle_ctx;
  sTicosDataSourceRleEncodeCtx encode_ctx;
} sTicosDataSourceRleState;

static sTicosDataSourceRleState s_ds_rle_state;

bool ticos_data_source_rle_encoder_set_active(const sTicosDataSourceImpl *source) {
  if (source == s_active_data_source) {
    return true;
  }

  s_ds_rle_state = (sTicosDataSourceRleState) { 0 };
...
static bool prv_data_source_rle_has_more_msgs_prepare(const void *data,
  ...
  while (bytes_encoded != data_len) {
    bytes_encoded += ticos_rle_encode(
  ...
  ticos_rle_encode_finalize(&s_ds_rle_state.rle_ctx);
  sTicosDataSourceRleEncodeCtx *encode_ctx = &s_ds_rle_state.encode_ctx;
  encode_ctx->state = kTicosDataSourceRleState_FindingSeqLength;
  return true;
}

static uint32_t prv_data_source_rle_get_backing_read_offset(void) {
  sTicosDataSourceRleEncodeCtx *encode_ctx = &s_ds_rle_state.encode_ctx;
  if (encode_ctx->state == kTicosDataSourceRleState_FindingSeqLength) {
    return encode_ctx->bytes_processed;
  }

  // else (encode_ctx->state == kTicosDataSourceRleState_WritingSequence)
  sTicosRleWriteInfo *write_info = &s_ds_rle_state.rle_ctx.write_info;
...
static bool prv_data_source_rle_read_msg_prepare(const void *data,
  ...
  sTicosRleCtx *rle_ctx = &s_ds_rle_state.rle_ctx;
  sTicosDataSourceRleEncodeCtx *encode_ctx = &s_ds_rle_state.encode_ctx;
  const size_t bytes_encoded = ticos_rle_encode(rle_ctx, data, data_len);
  if (data_len == 0) {
    ticos_rle_encode_finalize(rle_ctx);
  }
  encode_ctx->bytes_processed += bytes_encoded;

  sTicosRleWriteInfo *write_info = &s_ds_rle_state.rle_ctx.write_info;
  if (write_info->available) {
    encode_ctx->state = kTicosDataSourceRleState_WritingSequence;
  }
...
static uint32_t prv_data_source_rle_build_msg_incremental(uint8_t *buf,
  ...
  if (write_offset < write_info->header_len) {
    header_bytes_to_write =
        TICOS_MIN(buf_len, write_info->header_len - write_offset);
  ...
  size_t data_to_write =
      TICOS_MIN(buf_len, total_write_len - encode_ctx->write_offset);
  ...
  if (encode_ctx->write_offset == total_write_len) {
    ...
    encode_ctx->state = kTicosDataSourceRleState_FindingSeqLength;
    *write_info = (sTicosRleWriteInfo){0};
  }
...
static bool prv_data_source_rle_read(uint32_t offset, void *buf,
  ...
  while (encode_ctx->bytes_processed != s_ds_rle_state.original_size) {
    ...
    const size_t bytes_read = TICOS_MIN(bytes_remaining, working_buf_size);
...
static size_t prv_compute_rle_size(void) {
  ...
  while (bytes_processed != s_ds_rle_state.original_size) {
    ...
    const size_t bytes_to_read = TICOS_MIN(bytes_left, working_buf_size);
...
bool ticos_data_source_rle_has_more_msgs(size_t *total_size_out) {
...
const sTicosDataSourceImpl g_ticos_data_rle_source = {
  .has_more_msgs_cb = ticos_data_source_rle_has_more_msgs,
  .read_msg_cb = ticos_data_source_rle_read_msg,
  .mark_msg_read_cb = ticos_data_source_rle_mark_msg_read,
};

#endif /* TICOS_DATA_SOURCE_RLE_ENABLED */
