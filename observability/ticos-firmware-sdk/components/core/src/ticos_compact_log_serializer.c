...
TICOS_PUT_IN_SECTION(".log_fmt_hdr")
const sTicosLogFmtElfSectionHeader g_ticos_log_fmt_elf_section_hdr = {
...
bool ticos_vlog_compact_serialize(sTicosCborEncoder *encoder, uint32_t log_id,
  ...
  const size_t num_args = (31UL - TICOS_CLZ(compressed_fmt)) / bits_per_arg;

  if (!ticos_cbor_encode_array_begin(encoder, 1 /* log_id */ + num_args)) {
    return false;
  }
  ...
  if (!ticos_cbor_encode_unsigned_integer(encoder, log_fmt_offset)) {
    return false;
  }

  for (size_t i = 0; i < num_args; i++) {
    // See ticos/core/compact_log_helpers.h for more details. In essence,
    ...
    switch (type) {
      ...
      case TICOS_LOG_ARG_PROMOTED_TO_INT32: {
        int32_t val = va_arg(args, int32_t);
        success = ticos_cbor_encode_signed_integer(encoder, val);
        break;
      }

      case TICOS_LOG_ARG_PROMOTED_TO_INT64: {
        // We differentiate between an 64 bit and 32 bit integer arg
        // by packing the cbor encoded int in an array.
        uint64_t val = va_arg(args, uint64_t);
        success = ticos_cbor_encode_array_begin(encoder, 1) &&
            ticos_cbor_encode_long_signed_integer(encoder, (int64_t)val);
        break;
      }

      case TICOS_LOG_ARG_PROMOTED_TO_DOUBLE: {
        ...
        TICOS_STATIC_ASSERT(sizeof(uint64_t) == sizeof(double), "double does not fit in uint64_t");

        // Note: We use memcpy to properly type-pun / avoid strict-aliasing violation:
        //  https://ticos.io/strict-aliasing-type-punning
        double val = va_arg(args, double);
        uint64_t double_as_uint64;
        memcpy(&double_as_uint64, &val, sizeof(val));
        success = ticos_cbor_encode_uint64_as_double(encoder, double_as_uint64);
        break;
      }

      case TICOS_LOG_ARG_PROMOTED_TO_STR: {
        const char *str = va_arg(args, const char*);
        success = ticos_cbor_encode_string(encoder, str != NULL ? str : "(null)");
        break;
      }
...
bool ticos_log_compact_serialize(sTicosCborEncoder *encoder, uint32_t log_id,
  ...
  const bool success = ticos_vlog_compact_serialize(encoder, log_id, compressed_fmt, args);
  va_end(args);
