...
static sMfltLogDataSourceCtx s_ticos_log_data_source_ctx;
...
static bool prv_copy_msg_callback(sMfltLogIterator *iter, TICOS_UNUSED size_t offset,
                                  const char *buf, size_t buf_len) {
  sMfltLogEncodingCtx *const ctx = (sMfltLogEncodingCtx *)iter->user_ctx;
  return ticos_cbor_join(&ctx->encoder, buf, buf_len);
}

static bool prv_encode_current_log(sTicosCborEncoder *encoder, sMfltLogIterator *iter) {
...
static void prv_logs_mark_sent(void) {
  ...
  ticos_lock();
  s_ticos_log_data_source_ctx = (sMfltLogDataSourceCtx) { 0 };
  ticos_unlock();
}
