...
//! Ticos SDK into payloads that can be sent over the transport used up to the cloud
//!
...
//! ticos data packetizer. This way a user can easily add or remove provider functionality by
...
typedef bool (TicosDataSourceHasMoreMessagesCallback)(size_t *next_msg_size);
...
//! read goes past the next_msg_size returned from TicosDataSourceHasMoreMessagesCallback
typedef bool (TicosDataSourceReadMessageCallback)(uint32_t offset, void *buf, size_t buf_len);

//! Delete the currently queued up message being read
//!
//! A subsequent call to the paired TicosDataSourceHasMoreMessagesCallback will return
//! a info about a new message or nothing if there are no more messages to read
typedef void (TicosDataSourceMarkMessageReadCallback)(void);

typedef struct TicosDataSourceImpl {
  TicosDataSourceHasMoreMessagesCallback *has_more_msgs_cb;
  TicosDataSourceReadMessageCallback *read_msg_cb;
  TicosDataSourceMarkMessageReadCallback *mark_msg_read_cb;
} sTicosDataSourceImpl;

//! "Coredump" data source provided as part of "panics" component
extern const sTicosDataSourceImpl g_ticos_coredump_data_source;

//! Events (i.e "Heartbeat Metrics" & "Reset Reasons") provided as part of "metrics" and "panics"
//! component, respectively
extern const sTicosDataSourceImpl g_ticos_event_data_source;

//! Logging data source provided as part of "core" component (ticos/core/log.h)
extern const sTicosDataSourceImpl g_ticos_log_data_source;

//! Custom Data Recording (CDR) source provided as part of "core" component
//! (ticos/core/custom_data_recording.h)
extern const sTicosDataSourceImpl g_ticos_cdr_source;
