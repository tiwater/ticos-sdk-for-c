...
//!
//! All Ticos APIs return:
//!    0 on success
//!  < 0 on error
//!
//! This way a ticos API call can be checked with a simple "if (rv != 0) { }" to decide if
...
//! @brief
//!
//! Internal error codes used within the Ticos components. An external caller should never need
//! to use these Error codes.
typedef enum TicosInternalReturnCode {
  TicosInternalReturnCode_Error = -1,
  TicosInternalReturnCode_InvalidInput = -2,

  TicosReturnCode_PlatformBase = -1000
} TicosInternalReturnCode;
...
#define TICOS_PLATFORM_SPECIFIC_ERROR(e) \
  (TicosReturnCode_PlatformBase - ((int32_t)TICOS_ABS(e) & 0x7fffffff))
