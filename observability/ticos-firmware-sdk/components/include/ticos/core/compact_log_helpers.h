...
//! time via a static assert in "ticos/core/preprocessor.h"
...
#define TICOS_LOG_ARG_PROMOTED_TO_INT32 0
#define TICOS_LOG_ARG_PROMOTED_TO_INT64 1
#define TICOS_LOG_ARG_PROMOTED_TO_DOUBLE 2
#define TICOS_LOG_ARG_PROMOTED_TO_STR 3

#ifdef __cplusplus


#ifdef TICOS_UNITTEST
  //! Note: For trace_event Ticos CppUTest tests, we pick up this header but do not actually use
...
#define _TICOS_LOG_ARG_PROMOTION_TYPE(arg)                           \
  _Generic((arg) + 0,                                                   \
           float:  TICOS_LOG_ARG_PROMOTED_TO_DOUBLE,                 \
           double:  TICOS_LOG_ARG_PROMOTED_TO_DOUBLE,                \
           long double:  TICOS_LOG_ARG_PROMOTED_TO_DOUBLE,           \
           char*:  TICOS_LOG_ARG_PROMOTED_TO_STR,                    \
           const char*:  TICOS_LOG_ARG_PROMOTED_TO_STR,              \
           default: sizeof((arg) + 0) <= sizeof(int) ?                  \
              TICOS_LOG_ARG_PROMOTED_TO_INT32 : TICOS_LOG_ARG_PROMOTED_TO_INT64) \
...
#define TICOS_FMT_ENCODE_OP(_arg, _shift) (_TICOS_LOG_ARG_PROMOTION_TYPE(_arg) << _shift)
...
//!       KEEP(*(log_fmt))
//!    }
#define TICOS_LOG_FMT_ELF_SECTION TICOS_PUT_IN_SECTION("log_fmt")

//! The metadata we track in the ELF for each compact log in the code.
//! This is used to recover the original information
#define TICOS_LOG_FMT_ELF_SECTION_ENTRY(format, ...)                      \
  TICOS_LOG_FMT_ELF_SECTION static const char _ticos_log_fmt_ptr[] =   \
      TICOS_EXPAND_AND_QUOTE(TICOS_ARG_COUNT_UP_TO_32(__VA_ARGS__))";" \
      __FILE__ ";" TICOS_EXPAND_AND_QUOTE(__LINE__) ";" format


#define TICOS_LOG_FMT_ELF_SECTION_ENTRY_PTR \
  ((uint32_t)(uintptr_t)&_ticos_log_fmt_ptr[0])


typedef struct TicosLogFmtElfSectionHeader {
  uint32_t magic;
  uint8_t version;
  uint8_t rsvd[3];
} sTicosLogFmtElfSectionHeader;

extern const sTicosLogFmtElfSectionHeader g_ticos_log_fmt_elf_section_hdr;
