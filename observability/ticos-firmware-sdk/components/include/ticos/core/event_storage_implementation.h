...
struct TicosEventStorageImpl {
  //! Opens a session to begin writing a heartbeat event to storage
  //!
  //! @note To close the session, ticos_events_storage_finish_write() must be called
  //!
  ...
  //!  to "g_ticos_event_data_source.has_more_msgs_cb" will return the event. If true,
