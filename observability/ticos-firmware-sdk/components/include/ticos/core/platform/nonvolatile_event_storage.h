...
//!
//! Dependencies which must be implemented to persist events collected by the Ticos SDK into
...
typedef bool (TicosEventReadCallback)(uint32_t offset, void *buf, size_t buf_len);

typedef struct TicosNonVolatileEventStorageImpl {
  //! @return if true, the Ticos SDK will persist events here when
  //! "ticos_event_storage_persist" is called. if false, none of the other
  ...
  bool (*write)(TicosEventReadCallback reader_callback, size_t total_size);

} sTicosNonVolatileEventStorageImpl;
...
//!   ticos_event_storage_persist();
//! }
extern const sTicosNonVolatileEventStorageImpl g_ticos_platform_nv_event_storage_impl;
