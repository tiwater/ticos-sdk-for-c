//! @file
//!
//! Copyright (c) Ticos, Inc.
//! See License.txt for details
//!
//! @brief
//! Utility responsible for serializing out a Ticos Heartbeat. Today, the serialization format
//! used is cbor but the actual format used is opaque to an end user and no assumptions should be
//! made in user code based on it.

#include "ticos/metrics/serializer.h"

#include <stdio.h>
#include <stddef.h>
#include <string.h>

#include "ticos/core/compiler.h"
#include "ticos/core/debug_log.h"
#include "ticos/core/event_storage.h"
#include "ticos/core/event_storage_implementation.h"
#include "ticos/core/platform/device_info.h"
#include "ticos/core/serializer_helper.h"
#include "ticos/core/serializer_key_ids.h"
#include "ticos/metrics/utils.h"
#include "ticos/util/cbor.h"

typedef struct {
  sTicosCborEncoder encoder;
  bool compute_worst_case_size;
  bool encode_success;
} sTicosSerializerState;

static bool prv_metric_heartbeat_writer(void *ctx, const sTicosMetricInfo *metric_info) {
  sTicosSerializerState *state = (sTicosSerializerState *)ctx;
  sTicosCborEncoder *encoder = &state->encoder;

  // encode the value
  switch (metric_info->type) {
    case kTicosMetricType_Timer:
    case kTicosMetricType_Unsigned: {
      const uint32_t value = state->compute_worst_case_size ? UINT32_MAX : metric_info->val.u32;
      state->encode_success = ticos_cbor_encode_unsigned_integer(encoder, value);
      break;
    }
    case kTicosMetricType_Signed: {
      const int32_t value = state->compute_worst_case_size ? INT32_MIN : metric_info->val.i32;
      state->encode_success = ticos_cbor_encode_signed_integer(encoder, value);
      break;
    }
    case kTicosMetricType_String: {
      const char *value = metric_info->val.ptr;
      state->encode_success = ticos_cbor_encode_string(encoder, value);
      break;
    }
    case kTicosMetricType_NumTypes: // silence error with -Wswitch-enum
    default:
      break;
  }

  // only continue iterating if the encode was successful
  return state->encode_success;
}

static bool prv_serialize_latest_heartbeat_and_deinit(sTicosSerializerState *state) {
  bool success = false;

  sTicosCborEncoder *encoder = &state->encoder;
  if (!ticos_serializer_helper_encode_metadata(encoder, kTicosEventType_Heartbeat)) {
    goto cleanup;
  }

  // Encode up to "metrics:" section
  if (!ticos_cbor_encode_unsigned_integer(encoder, kTicosEventKey_EventInfo) ||
      !ticos_cbor_encode_dictionary_begin(encoder, 1) ||
      !ticos_cbor_encode_unsigned_integer(encoder, kTicosHeartbeatInfoKey_Metrics) ||
      !ticos_cbor_encode_array_begin(encoder, ticos_metrics_heartbeat_get_num_metrics())) {
    goto cleanup;
  }

  ticos_metrics_heartbeat_iterate(prv_metric_heartbeat_writer, state);
  success = state->encode_success;

cleanup:
  return success;
}

static bool prv_encode_cb(TICOS_UNUSED sTicosCborEncoder *encoder, void *ctx) {
  sTicosSerializerState *state = (sTicosSerializerState *)ctx;
  return prv_serialize_latest_heartbeat_and_deinit(state);
}

size_t ticos_metrics_heartbeat_compute_worst_case_storage_size(void) {
  sTicosSerializerState state = { .compute_worst_case_size = true };
  return ticos_serializer_helper_compute_size(&state.encoder, prv_encode_cb, &state);
}

bool ticos_metrics_heartbeat_serialize(const sTicosEventStorageImpl *storage_impl) {
  // Build a heartbeat event, which looks like this:
  // {
  //    "type": "heartbeat",
  //    "device_serial": "DAABBCCDD",
  //    "software_type": "main",
  //    "software_version": "1.2.3",
  //    "hardware_version": "evt_24",
  //    "event_info": {
  //         "metrics": {
  //          ... heartbeat metrics ...
  //    }
  // }
  // NOTE: "sdk_version" is not included, but derived from the CborSchemaVersion

  // NOTE: We'll always attempt to serialize the heartbeat and rollback if we are out of space
  // avoiding the need to serialize the data twice
  sTicosSerializerState state = { 0 };
  const bool success = ticos_serializer_helper_encode_to_storage(
      &state.encoder, storage_impl, prv_encode_cb, &state);

  return success;
}
