...
typedef struct {
  bool md;
  bool continuation;
} sTicosHeaderSettings;

static uint8_t prv_build_hdr(const sTicosHeaderSettings *settings) {
...
bool ticos_chunk_transport_get_next_chunk(sTcsChunkTransportCtx *ctx,
  ...
  if (*out_buf_len < TICOS_MIN_CHUNK_BUF_LEN) {
    *out_buf_len = 0;
    return true;
  }
  ...
  if (init_pkt_type) {
    ...
    if (init_settings.md) {
      ...
      bytes_to_read = TICOS_MIN(*out_buf_len - 1 /* hdr */, ctx->total_size);
    } else {
      bytes_to_read = ctx->total_size;
    }
  } else if (ctx->enable_multi_call_chunk) {
    const size_t bytes_remaining = ctx->total_size - ctx->read_offset;
    bytes_to_read = TICOS_MIN(*out_buf_len, bytes_remaining);
    const size_t out_buf_space_rem = *out_buf_len - bytes_to_read;
    more_data = (out_buf_space_rem < crc16_len);
  } else {
    varint_len = ticos_encode_varint_u32(ctx->read_offset, &chunk_msg[1]);
    const size_t bytes_remaining = ctx->total_size - ctx->read_offset;

    size_t out_buf_space_rem = *out_buf_len - 1 /* hdr */ - varint_len;
    bytes_to_read = TICOS_MIN(out_buf_space_rem, bytes_remaining);
    out_buf_space_rem -= bytes_to_read;
    more_data = (out_buf_space_rem < crc16_len);
    const sTicosHeaderSettings cont_settings = {
      .md = more_data,
      .continuation = true,
    };
  ...
  if (bytes_to_read != 0) {
    ...
    ctx->crc16_incremental = ticos_crc16_ccitt_compute(
  ...
  if (buf_space_rem != 0) {
    ...
    //
    // The Ticos backend allows for the last chunk in a message to exceed the
