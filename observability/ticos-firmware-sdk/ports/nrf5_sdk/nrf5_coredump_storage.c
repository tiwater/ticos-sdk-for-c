...
//!     TICOS_CORES (rw) : ORIGIN = 0xA1000, LENGTH = 0x40000
...
//!   }
//!
//!   __TicosCoreStorageStart = ORIGIN(TICOS_CORES);
//!   __TicosCoreStorageEnd = ORIGIN(TICOS_CORES) + LENGTH(TICOS_CORES);
//!
...
typedef enum {
  kTicosCoredumpClearState_Idle = 0,
  kTicosCoredumpClearState_ClearRequested,
  kTicosCoredumpClearState_ClearInProgress,
} eTicosCoredumpClearState;

static eTicosCoredumpClearState s_coredump_clear_state;

static void prv_handle_flash_op_complete(bool success) {
  if (s_coredump_clear_state == kTicosCoredumpClearState_Idle) {
    return; // not a flash op we care about
  }

  if (s_coredump_clear_state == kTicosCoredumpClearState_ClearInProgress &&
      success) {
    // The erase is complete!
    s_coredump_clear_state = kTicosCoredumpClearState_Idle;
    return;
  }
...
void ticos_platform_coredump_storage_clear(void) {
  ...
  if (nrf_sdh_is_enabled()) {
    ...
    s_coredump_clear_state = kTicosCoredumpClearState_ClearRequested;
    const uint32_t rv = sd_flash_write((void *)TICOS_COREDUMP_STORAGE_START_ADDR, &invalidate, 1);
    if (rv == NRF_SUCCESS) {
      s_coredump_clear_state = kTicosCoredumpClearState_ClearInProgress;
    ...
    nrf_nvmc_write_word(TICOS_COREDUMP_STORAGE_START_ADDR, invalidate);
  }
}
...
//!
//! We implement this in ticos_coredump_read() so the logic is only run
//! while the system is running.
bool ticos_coredump_read(uint32_t offset, void *data, size_t read_len) {
  if (s_coredump_clear_state != kTicosCoredumpClearState_Idle) {
