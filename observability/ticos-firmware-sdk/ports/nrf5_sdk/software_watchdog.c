...
//!  #define TICOS_NRF5_WATCHDOG_SW_AUTOCONFIG 0
//!  #define TICOS_WATCHDOG_SW_TIMEOUT_SECS <desired timeout in seconds>
//!
//! Recommended setup:
//!
...
//!      ${TICOS_SDK_ROOT}/ports/nrf5_sdk/software_watchdog.c
...
#define TICOS_PRESCALE_RESOLUTION_MS 125


#ifndef TICOS_NRF5_WATCHDOG_SW_AUTOCONFIG
//! Auto configure software watchdog timeout relative to hardware watchdog
#define TICOS_NRF5_WATCHDOG_SW_AUTOCONFIG NRFX_WDT_ENABLED
#endif

#if TICOS_NRF5_WATCHDOG_SW_AUTOCONFIG

#if NRFX_WDT_CONFIG_RELOAD_VALUE < TICOS_PRESCALE_RESOLUTION_MS
#error "NRFX_WDT_CONFIG_RELOAD_VALUE must be > TICOS_PRESCALE_RESOLUTION_MS (125ms)"
#endif

#define TICOS_NRF5_WATCHDOG_SW_TIMEOUT_MS \
  (NRFX_WDT_CONFIG_RELOAD_VALUE - TICOS_PRESCALE_RESOLUTION_MS)

#else

#if (NRFX_WDT_ENABLED && (NRFX_WDT_CONFIG_RELOAD_VALUE < (TICOS_WATCHDOG_SW_TIMEOUT_SECS * 1000)))
#error "Set TICOS_WATCHDOG_SW_TIMEOUT_SECS must be less than hardware watchdog timeout (NRFX_WDT_CONFIG_RELOAD_VALUE)"
#endif

#define TICOS_NRF5_WATCHDOG_SW_TIMEOUT_MS  (TICOS_WATCHDOG_SW_TIMEOUT_SECS * 1000)

#endif /* TICOS_NRF5_WATCHDOG_SW_AUTOCONFIG */

#ifndef TICOS_NRF5_WATCHDOG_SW_CHANNEL
#define TICOS_NRF5_WATCHDOG_SW_CHANNEL 0
#endif
...
static void prv_software_watchdog_timeout(nrfx_rtc_int_type_t int_type) {
  ...
  //! Note: We can't call TICOS_EXC_HANDLER_WATCHDOG directly here because the IRQ is handled by
...
int ticos_software_watchdog_enable(void) {
  ...
  if (err_code != NRF_SUCCESS) {
    TICOS_LOG_ERROR("Failed to configure rtc software watchdog: rv=%d", (int)err_code);
    return err_code;
  }
  ...
  ticos_software_watchdog_update_timeout(TICOS_NRF5_WATCHDOG_SW_TIMEOUT_MS);
  return 0;
}
...
int ticos_software_watchdog_update_timeout(uint32_t timeout_ms) {
  const uint32_t counter_val = (timeout_ms / TICOS_PRESCALE_RESOLUTION_MS);
  TICOS_LOG_INFO("Software Watchdog configured to %dms",
                    (int)(counter_val * TICOS_PRESCALE_RESOLUTION_MS));
  ...
  nrfx_rtc_cc_set(&s_rtc, TICOS_NRF5_WATCHDOG_SW_CHANNEL, counter_val, true /* enable_irq */);
