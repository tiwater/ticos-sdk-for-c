...
void ticos_platform_reboot_tracking_boot(void) {
  ...
  ticos_reboot_tracking_boot((void *)reboot_tracking_start_addr, &reset_reason);
}

// Map chip-specific reset reasons to Ticos reboot reasons.
void ticos_reboot_reason_get(sResetBootupInfo *info) {
  ...
  TICOS_PRINT_RESET_INFO("Reset Reason, RESETREAS=0x%" PRIx32, reset_reason_reg);
