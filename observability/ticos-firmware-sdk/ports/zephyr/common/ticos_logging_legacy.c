...
//!
//! @brief
//! Hooks the Ticos logging API to zephyr's "V1" logging system.
...
const struct log_backend_api log_backend_tcs_api = {
  ...
  // Note: We don't want to clutter Ticos circular buffer with hex dumps
...
static int prv_log_out(uint8_t *data, size_t length, void *ctx) {
  ...
  const eTicosPlatformLogLevel log_level = ctx != NULL ? *(eTicosPlatformLogLevel*)ctx :
                                                            kTicosPlatformLogLevel_Error;
  ticos_log_save_preformatted(log_level, data, length);
  return (int) length;
}
...
static void prv_log_put(const struct log_backend *const backend, struct log_msg *msg) {
  ...
  if (zephyr_level != LOG_LEVEL_NONE) {
    // We might log so figure out if Ticos logging level for when Zephyr calls us back.
    eTicosPlatformLogLevel ticos_level = prv_map_zephyr_level_to_ticos(zephyr_level);
    log_output_ctx_set(&s_log_output_tcs, &ticos_level);
    log_output_msg_process(&s_log_output_tcs, msg, flags);
  }
  log_msg_put(msg);
}
...
static void prv_log_put_sync_string(const struct log_backend *const backend,
  ...
  // we use ticos logging directly instead
  eTicosPlatformLogLevel ticos_level = prv_map_zephyr_level_to_ticos(src_level.level);
  ticos_vlog_save(ticos_level, fmt, ap);
}
