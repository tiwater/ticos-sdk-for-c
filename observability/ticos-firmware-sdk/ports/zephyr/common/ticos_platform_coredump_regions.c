...
#if TICOS_ZEPHYR_VERSION_GT(2, 1)
...
static sTcsCoredumpRegion s_coredump_regions[
    ...
    + 2 /* active stack(s) */
    + 1 /* _kernel variable */
#if CONFIG_TICOS_COREDUMP_COLLECT_DATA_REGIONS
    + 1
#endif
#if CONFIG_TICOS_COREDUMP_COLLECT_BSS_REGIONS
    + 1
#endif
    ];
...
const sTcsCoredumpRegion *ticos_platform_coredump_get_regions(
  ...
  size_t stack_size_to_collect = ticos_platform_sanitize_address_range(
        crash_info->stack_address, CONFIG_TICOS_COREDUMP_STACK_SIZE_TO_COLLECT);
  ...
  if (msp_was_active) {
    ...
    stack_size_to_collect = ticos_platform_sanitize_address_range(
        psp, CONFIG_TICOS_COREDUMP_STACK_SIZE_TO_COLLECT + extra_stack_bytes);
    s_coredump_regions[region_idx] = TICOS_COREDUMP_MEMORY_REGION_INIT(
  ...
  region_idx += ticos_zephyr_get_task_regions(
...
#if CONFIG_TICOS_COREDUMP_COLLECT_DATA_REGIONS
  region_idx +=  ticos_zephyr_get_data_regions(
      &s_coredump_regions[region_idx], TICOS_ARRAY_SIZE(s_coredump_regions) - region_idx);
#endif

#if CONFIG_TICOS_COREDUMP_COLLECT_BSS_REGIONS
  region_idx +=  ticos_zephyr_get_bss_regions(
      &s_coredump_regions[region_idx], TICOS_ARRAY_SIZE(s_coredump_regions) - region_idx);
#endif
