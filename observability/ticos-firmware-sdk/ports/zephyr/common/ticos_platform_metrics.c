...
static TicosPlatformTimerCallback *s_metrics_timer_callback;
...
static void prv_execution_cycles_delta_update(TicosMetricId key, uint64_t curr_cycles,
  ...
  ticos_metrics_heartbeat_set_unsigned(key, heartbeat_cycles);
  *prev_cycles = curr_cycles;
}
#endif

// Written as a function vs. in-line b/c we might want to extern this at some point?
// See ports/zephyr/config/ticos_metrics_heartbeat_zephyr_port_config.def for
...
void ticos_metrics_heartbeat_collect_sdk_data(void) {
...
#if defined(CONFIG_THREAD_STACK_INFO) && TICOS_ZEPHYR_VERSION_GT(2, 1)
  ...
  ticos_metrics_heartbeat_set_unsigned(
      TICOS_METRICS_KEY(TimerTaskFreeStack), free_stack_size);
#endif
  ...
  prv_execution_cycles_delta_update(TICOS_METRICS_KEY(TimerTaskCpuUsage),
                                    rt_stats_thread.execution_cycles,
                                    &s_prev_thread_execution_cycles);

  k_thread_runtime_stats_t rt_stats_all;
  k_thread_runtime_stats_all_get(&rt_stats_all);
  prv_execution_cycles_delta_update(TICOS_METRICS_KEY(AllTasksCpuUsage),
  ...
  ticos_metrics_heartbeat_set_unsigned(TICOS_METRICS_KEY(Heap_BytesFree), stats.free_bytes);
#endif /* defined(CONFIG_SYS_HEAP_RUNTIME_STATS) */

#endif /* CONFIG_TICOS_METRICS_DEFAULT_SET_ENABLE */
}
