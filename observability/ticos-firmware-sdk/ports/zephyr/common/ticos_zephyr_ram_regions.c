...
#if !defined(TICOS_ZEPHYR_USE_OLD_DATA_REGION_NAMES)
#define TICOS_ZEPHYR_USE_OLD_DATA_REGION_NAMES 0
#endif

static struct k_thread *s_task_tcbs[CONFIG_TICOS_COREDUMP_MAX_TRACKED_TASKS];
#define EMPTY_SLOT 0

static bool prv_find_slot(size_t *idx, struct k_thread *desired_tcb) {
  for (size_t i = 0; i < TICOS_ARRAY_SIZE(s_task_tcbs); i++) {
...
size_t ticos_zephyr_get_task_regions(sTcsCoredumpRegion *regions, size_t num_regions) {
  ...
  for (size_t i = 0; i < TICOS_ARRAY_SIZE(s_task_tcbs) && region_idx < num_regions; i++) {
    ...
    regions[region_idx] = TICOS_COREDUMP_MEMORY_REGION_INIT(thread, tcb_size);
    region_idx++;
  }
  ...
  for (size_t i = 0; i < TICOS_ARRAY_SIZE(s_task_tcbs) && region_idx < num_regions; i++) {
    ...
    // the area we need to collect if less than CONFIG_TICOS_COREDUMP_STACK_SIZE_TO_COLLECT
    // is in use
    const uint32_t stack_top = thread->stack_info.start + thread->stack_info.size;
    size_t stack_size_to_collect =
        TICOS_MIN(stack_top - (uint32_t)sp, CONFIG_TICOS_COREDUMP_STACK_SIZE_TO_COLLECT);
#else
    size_t stack_size_to_collect = CONFIG_TICOS_COREDUMP_STACK_SIZE_TO_COLLECT;
#endif

    stack_size_to_collect = ticos_platform_sanitize_address_range(sp, stack_size_to_collect);
    if (stack_size_to_collect == 0) {
      continue;
    }

    regions[region_idx] = TICOS_COREDUMP_MEMORY_REGION_INIT(sp, stack_size_to_collect);
    region_idx++;
  }
...
size_t ticos_zephyr_get_data_regions(sTcsCoredumpRegion *regions, size_t num_regions) {
  ...
  // with a Ticos SDK version >=0.27.3, because that NCS release used an
  // intermediate Zephyr release, so the version number checking is not
  // possible.
#if !TICOS_ZEPHYR_USE_OLD_DATA_REGION_NAMES && TICOS_ZEPHYR_VERSION_GT(2, 6)
...
#define ZEPHYR_DATA_REGION_END __data_ram_end
  ...
  regions[0] = TICOS_COREDUMP_MEMORY_REGION_INIT(ZEPHYR_DATA_REGION_START, size_to_collect);
  return 1;
}
...
size_t ticos_zephyr_get_bss_regions(sTcsCoredumpRegion *regions, size_t num_regions) {
  ...
  regions[0] = TICOS_COREDUMP_MEMORY_REGION_INIT(__bss_start, size_to_collect);
  return 1;
}
