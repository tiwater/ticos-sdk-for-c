...
//!
//! 3) Implement ticos_platform_sanitize_address_range(). This routine is used
...
#if !defined(TICOS_FREERTOS_TRACE_ENABLED)
#error "'#include "ticos/ports/freertos_trace.h"' must be added to FreeRTOSConfig.h"
#endif
...
//! TICOS_FREERTOS_TCB_SIZE to ticos_platform_config.h
#if !defined(TICOS_FREERTOS_TCB_SIZE)
#define TICOS_FREERTOS_TCB_SIZE 200
#endif
#else
#define TICOS_FREERTOS_TCB_SIZE sizeof(StaticTask_t)
#endif

//! The maximum number
static void *s_task_tcbs[TICOS_PLATFORM_MAX_TRACKED_TASKS];
#define EMPTY_SLOT 0

static bool prv_find_slot(size_t *idx, void *desired_tcb) {
  for (size_t i = 0; i < TICOS_PLATFORM_MAX_TRACKED_TASKS; i++) {
...
void ticos_freertos_trace_task_create(void *tcb) {
  ...
  if (!slot_found) {
    TICOS_LOG_ERROR("Task registry full (%d)", TICOS_PLATFORM_MAX_TRACKED_TASKS);
  }
}
...
size_t ticos_freertos_get_task_regions(sTcsCoredumpRegion *regions, size_t num_regions) {
  ...
  for (size_t i = 0; i < TICOS_ARRAY_SIZE(s_task_tcbs) && region_idx < num_regions; i++) {
    ...
    regions[region_idx] = TICOS_COREDUMP_MEMORY_REGION_INIT(tcb_address, tcb_size);
    region_idx++;
  }
  ...
  for (size_t i = 0; i < TICOS_ARRAY_SIZE(s_task_tcbs) && region_idx < num_regions; i++) {
    ...
    regions[region_idx] =  TICOS_COREDUMP_MEMORY_REGION_INIT(top_of_stack, stack_size);
    region_idx++;
