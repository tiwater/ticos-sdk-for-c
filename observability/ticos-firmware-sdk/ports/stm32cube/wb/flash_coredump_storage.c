...
#include "ticos/ports/stm32cube/wb/flash.h"
...
bool ticos_stm32cubewb_flash_clear_ecc_errors(
...
void ticos_platform_fault_handler(const sTcsRegState *regs, eTicosRebootReason reason) {
  ticos_stm32cubewb_flash_clear_ecc_errors(
...
bool ticos_platform_coredump_storage_erase(uint32_t offset, size_t erase_size) {
  ...
  const uint32_t erase_begin_addr = TICOS_COREDUMP_STORAGE_START_ADDR + offset;
