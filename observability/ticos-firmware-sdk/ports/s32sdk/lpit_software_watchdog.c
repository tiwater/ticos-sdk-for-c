...
//!
//! By setting TICOS_WATCHDOG_SW_TIMEOUT_SECS to a timeout less than the hardware watchdog, we
...
# define TICOS_SOFWARE_WATCHDOG_SOURCE 0
#endif
...
#ifndef TICOS_SOFWARE_WATCHDOG_SOURCE_CLOCK_FREQ
# define TICOS_SOFWARE_WATCHDOG_SOURCE_CLOCK_FREQ 0
#endif

#if TICOS_SOFWARE_WATCHDOG_SOURCE < 0 || TICOS_SOFWARE_WATCHDOG_SOURCE > 4
# error "TICOS_SOFWARE_WATCHDOG_SOURCE must be between 0 and 3"
#endif

#ifndef TICOS_EXC_HANDLER_WATCHDOG
# if TICOS_SOFWARE_WATCHDOG_SOURCE == 0
#  error "Port expects following define to be set: -DTICOS_EXC_HANDLER_WATCHDOG=LPIT0_Ch0_IRQHandler"
# else
#  error "Port expects following define to be set: -DTICOS_EXC_HANDLER_WATCHDOG=LPIT0_Ch<N>_IRQHandler"
# endif
#endif
...
static int prv_lpit_with_timeout(uint32_t timeout_ms) {
  ...
  switch (pcs) {
    case 1:
      ...
      src_clock_freq = TICOS_SOFWARE_WATCHDOG_SOURCE_CLOCK_FREQ;
      break;
    case 2:
      clock_name = "SIRCDIV2";
    ...
    case 6:
      ...
      src_clock_freq = TICOS_SOFWARE_WATCHDOG_SOURCE_CLOCK_FREQ;
    default:
      TICOS_LOG_ERROR("Illegal clock source for LPIT. PCC=0x%" PRIx32" PCS=0x%" PRIx32, pcc, pcs);
      return -1;
  }

  if (clock_div2 == 0) {
    TICOS_LOG_ERROR("LPIT source clock (%s) not enabled", clock_name);
    return -2;
  }

  if (src_clock_freq == 0) {
    TICOS_LOG_ERROR("-DTICOS_SOFWARE_WATCHDOG_SOURCE_CLOCK_FREQ=<freq_hz> required");
    return -3;
  }

  const uint32_t clock_freq_hz = src_clock_freq >> (clock_div2 - 1);

  TICOS_LOG_DEBUG("Configuring SW Watchdog. Source=%s, Timeout=%dms, Src Clock=%dHz",
  ...
  if (desired_tval > UINT32_MAX) {
    ...
    TICOS_LOG_ERROR("Can't configure software watchdog of %d sec. Max=%"PRIu32" sec",
                       TICOS_WATCHDOG_SW_TIMEOUT_SECS, max_seconds);
    return -4;
  }
  ...
  LPIT0->TMR[TICOS_SOFWARE_WATCHDOG_SOURCE].TCTRL &= ~LPIT_TMR_TCTRL_T_EN(1);

  // Set up the countdown to match the desired watchdog timeout
  LPIT0->TMR[TICOS_SOFWARE_WATCHDOG_SOURCE].TVAL = (uint32_t)desired_tval;
  ...
  LPIT0->TMR[TICOS_SOFWARE_WATCHDOG_SOURCE].TCTRL =
...
int ticos_software_watchdog_enable(void) {
  const uint32_t timeout_ms = \
      TICOS_WATCHDOG_SW_TIMEOUT_SECS * 1000;
