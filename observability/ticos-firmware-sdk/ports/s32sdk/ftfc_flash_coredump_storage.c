...
//! }
//! __TicosCoreStorageStart = ORIGIN(m_flexnvm);
//! __TicosCoreStorageEnd = ORIGIN(m_flexnvm) + LENGTH(m_flexnvm);
//!
//! Notes:
...
#define TICOS_S32_PF_BASE 0x00000000U
#define TICOS_S32_PF_END (TICOS_S32_PF_BASE + FEATURE_FLS_PF_BLOCK_SIZE)

#define TICOS_S32_DF_BASE 0x10000000U
#define TICOS_S32_DF_END (TICOS_S32_DF_BASE + FEATURE_FLS_DF_BLOCK_SIZE)

#define TICOS_S32_PROG_PHRASE_LEN 8
...
static bool prv_lookup_flash_info(uint32_t start_addr, uint32_t end_addr,
  ...
  if ((start_addr >= TICOS_S32_PF_BASE) && (end_addr <= TICOS_S32_PF_END)) {
    *sector_size = FEATURE_FLS_PF_BLOCK_SECTOR_SIZE;
    *flash_addr = start_addr;
  } else if ((start_addr >= TICOS_S32_DF_BASE) && (end_addr <= TICOS_S32_DF_END)) {
    // FlexNVM is memory mapped to start at 0x10000000 (TICOS_S32_DF_BASE) but the flash
    ...
    *flash_addr =  start_addr + 0x800000U - TICOS_S32_DF_BASE;
  } else {
...
static void prv_flash_clear_errors(void) {
  FTFC->FSTAT = TICOS_FSTAT_ERR_MASK;
}
...
static bool prv_erase_sector(uint32_t flash_address) {
  ...
  return ((FTFC->FSTAT & TICOS_FSTAT_ERR_MASK) == 0);
}
...
static void prv_write_double_word(uint32_t flash_address, uint8_t data[TICOS_S32_PROG_PHRASE_LEN]) {
  prv_flash_wait_for_ready();
  prv_flash_clear_errors();
  ...
  for (size_t i = 0; i < TICOS_S32_PROG_PHRASE_LEN ; i++) {
  ...
  if ((FTFC->FSTAT & TICOS_FSTAT_ERR_MASK) != 0) {
    prv_coredump_writer_assert_and_reboot(FTFC->FSTAT);
  }
}

bool ticos_platform_coredump_storage_buffered_write(sCoredumpWorkingBuffer *blk) {
  const uint32_t start_addr = TICOS_COREDUMP_STORAGE_START_ADDR + blk->write_offset;
  ...
  if ((block_size % TICOS_S32_PROG_PHRASE_LEN) != 0) {
    // configuration error
    prv_coredump_writer_assert_and_reboot(block_size);
  }

  for (size_t i = 0; i < block_size; i += TICOS_S32_PROG_PHRASE_LEN) {
...
void ticos_platform_coredump_storage_clear(void) {
  const uint32_t start_addr = TICOS_COREDUMP_STORAGE_START_ADDR;
  const uint32_t end_addr = TICOS_COREDUMP_STORAGE_END_ADDR;
  ...
  if (!prv_erase_sector(flash_address)) {
    TICOS_LOG_ERROR("Failed to clear coredump storage, 0x%" PRIx32, (uint32_t)FTFC->FSTAT);
  }
}
