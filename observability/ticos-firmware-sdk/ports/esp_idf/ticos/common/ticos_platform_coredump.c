...
//!
//! Port of the Ticos SDK to the esp-idf for esp32 devices
...
#error "Ticos SDK integration requires CONFIG_ESP32_ENABLE_COREDUMP_TO_FLASH=y sdkconfig setting"
#endif
...
#if CONFIG_TICOS_COREDUMP_USE_OTA_SLOT
...
const sTcsCoredumpRegion *ticos_platform_coredump_get_regions(
  ...
  if (core_part != NULL) {
    ...
    dram_collection_len = TICOS_MIN((core_part->size * 7) / 8,
                                       dram_collection_len);
  }
...
void __wrap_esp_core_dump_init(void) {
  ...
  if (core_part == NULL) {
    TICOS_LOG_ERROR("Coredumps enabled but no partition exists!");
    TICOS_LOG_ERROR("Add \"coredump\" to your partition.csv file");
    return;
  }

  TICOS_LOG_INFO("Coredumps will be saved to '%s' partition",
                    core_part->label);
  TICOS_LOG_INFO("Using entry %p pointing to address 0x%08" PRIX32,
...
esp_err_t __wrap_esp_core_dump_image_get(size_t* out_addr, size_t *out_size) {
  ...
  if (!ticos_coredump_has_valid_coredump(out_size)) {
...
void ticos_platform_coredump_storage_clear(void) {
  ...
  const esp_err_t err = ticos_esp_spi_flash_write(core_part->address, &invalidate,
                                                     sizeof(invalidate));
  if (err != ESP_OK) {
    ticos_platform_log(kTicosPlatformLogLevel_Error,
...
static void prv_panic_safe_putchar(char c) {
  ...
  bool ready = false;
  while (i++ < 1000 && !ready) {
    const uint32_t status = READ_PERI_REG(UART_STATUS_REG(TICOS_ESP32_CONSOLE_UART_NUM));
    ready = ((status >> UART_TXFIFO_CNT_S) & UART_TXFIFO_CNT) >= 126;
  }
  WRITE_PERI_REG(UART_FIFO_REG(TICOS_ESP32_CONSOLE_UART_NUM), c);
}
...
bool ticos_platform_coredump_save_begin(void) {
  ...
  prv_panic_safe_putstr("Saving Ticos Coredump!\r\n");
  return (ticos_esp_spi_flash_coredump_begin() == 0);
}
...
bool ticos_platform_coredump_storage_write(uint32_t offset, const void *data,
  ...
  const esp_err_t err = ticos_esp_spi_flash_write(address, data, data_len);
  if (err != ESP_OK) {
...
bool ticos_platform_coredump_storage_read(uint32_t offset, void *data,
  ...
  const esp_err_t err = ticos_esp_spi_flash_read(address, data, read_len);
  return (err == ESP_OK);
}
...
bool ticos_platform_coredump_storage_erase(uint32_t offset, size_t erase_size) {
  ...
  const size_t address = core_part->address + offset;
  const esp_err_t err = ticos_esp_spi_flash_erase_range(address, erase_size);
  if (err != ESP_OK) {
