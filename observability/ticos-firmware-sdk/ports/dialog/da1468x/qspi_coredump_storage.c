...
//! 1. Add "#define TICOS_PLATFORM_COREDUMP_STORAGE_USE_FLASH 1" to ticos_platform_config.h
//! 1. call ad_nvms_init() before using the functions in this API
//! 2. patch the Dialog SDK with the Ticos QSPI coredump storage patch,
//!    e.g. patch -p1 < /path/to/ticos_qspi_coredump_storage.patch
//! 3. call ticos_platform_coredump_storage_boot() from your ticos_platform_boot() implementation
...
#ifndef TICOS_PLATFORM_COREDUMP_STORAGE_PARTITION
#define TICOS_PLATFORM_COREDUMP_STORAGE_PARTITION NVMS_LOG_PART
#endif
...
#define TICOS_PLATFORM_COREDUMP_STORAGE_MAX_SIZE_BYTES UINT32_MAX
#endif
...
static bool s_ticos_using_qspi;
...
// coredump storage be sure to call this function early, e.g. from ticos_platform_boot().
...
void ticos_platform_coredump_storage_boot(void) {
  partition_entry_t partition_info = {0};
  const bool exists = ad_nvms_get_partition_info(TICOS_PLATFORM_COREDUMP_STORAGE_PARTITION, &partition_info);
  if (!exists) {
    TICOS_LOG_ERROR("Could not locate partition for coredump storage, has ad_nvms_init() been called?");
    return;
  }

  // ad_nvms_open() should not fail if above check succeeds.
  s_qspi_coredump_partition_info.partition.handle = ad_nvms_open(TICOS_PLATFORM_COREDUMP_STORAGE_PARTITION);
  const size_t partition_size = ad_nvms_get_size(s_qspi_coredump_partition_info.partition.handle);
  s_qspi_coredump_partition_info.partition.size = TICOS_MIN(
      partition_size, TICOS_PLATFORM_COREDUMP_STORAGE_MAX_SIZE_BYTES);
...
bool ticos_platform_saving_coredump(void) {
  return s_ticos_using_qspi;
}

// We need to override the weak nop version of ticos_platform_coredump_save_begin()
...
bool ticos_platform_coredump_save_begin(void) {
  bool ticos_ad_flash_is_locked(void); // Defined in ad_flash.c, not in a header file
  bool ticos_ad_nvms_direct_is_locked(void); // Defined in ad_nvms_direct.c, not in a header file
  if (ticos_ad_flash_is_locked() || ticos_ad_nvms_direct_is_locked()) {
    return false;
  }
  ...
  if (ticos_software_watchdog_feed() != 0) {
    return false;
  }
...
void ticos_platform_coredump_storage_clear(void) {
  ticos_platform_coredump_storage_erase(0, ad_nvms_erase_size());
}
