...
//!
//! A port of the Ticos Diagnostic GATT Service (MDS) to the DA1469x SDK.
//! See mds.h header for more details

#include "ticos/ports/ble/mds.h"
...
TICOS_STATIC_ASSERT(configTIMER_TASK_STACK_DEPTH >= 256,
...
#ifndef MDS_URI_BASE
//! i.e https://chunks.ticos.com/api/v0/chunks/
#define MDS_URI_BASE \
  (TICOS_HTTP_APIS_DEFAULT_SCHEME "://" TICOS_HTTP_CHUNKS_API_HOST "/api/v0/chunks/")
#endif
...
static void prv_try_notify(md_service_t *mds, uint16_t conn_idx) {
  ...
  if ((mds->payload == NULL) && ticos_packetizer_data_available()) {
    ...
    if (mds->payload != NULL) {
      ...
      ticos_packetizer_get_chunk(&mds->payload->chunk[0], &mds->chunk_len);
    }
  }
...
//! is any new data to send to Ticos while connected
static void prv_mds_timer_callback(TICOS_UNUSED TimerHandle_t handle) {
  if (s_mds == NULL) {
...
#endif /* defined(dg_configUSE_TICOS) */
